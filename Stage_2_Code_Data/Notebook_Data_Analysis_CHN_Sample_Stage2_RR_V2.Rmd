...
df_CHN_Journal_Gender <- df_stage2_Journal_Code_Gender_integer %>% 
  dplyr::summarise(male = sum(Male_Num)/sum(Sample_Size),
          female =sum(Female_Num)/sum(Sample_Size)) %>% 
  dplyr::mutate(male = round(male,2),
                female = round( female,2),
                Data_Source = "CHN Journal data") %>% 
  tidyr::pivot_longer(c(male, female), 
                      names_to = "Gender",
                      values_to = "Proportion")

sum(df_stage2_Journal_Code_Gender_decimal$Male_Num)/sum(df_stage2_Journal_Code_Gender_decimal$Male_Num+df_stage2_Journal_Code_Gender_decimal$Female_Num)
