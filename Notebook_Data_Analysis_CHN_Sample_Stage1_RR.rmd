...
pacman::p_load("tidyverse", "ggridges", "openxlsx", "ggh4x", "stringr", "patchwork", "BayesFactor")
library(tidyverse)
library(ggridges)
library(openxlsx)
library(ggh4x)
...
## additional BF for sensitivity:
# BF_h2a_sex_psych_null <- BayesMultiNomial(dataset = df_sex_ratio_jasp, factor = "Sex", observed = "Census7", expected = "Census7")
...
# BF_h2a_sex_psych_2_null <- BayesMultiNomial(dataset = df_sex_ratio_jasp, factor = "Sex", observed = "Census7", expected = "Census7", prior = "Census7")

# BF_h2a_sex_psych_2 <- BayesMultiNomial(dataset = df_sex_ratio_jasp, factor = "Sex", observed = "PSA001", expected = "Census7", default_prior = FALSE, prior = array(c(50, 50)))

## calculate the BF using contingency table:
# BF_h2a_sex_psych_3 <- BayesFactor::contingencyTableBF(as.matrix(df_sex_ratio_jasp[, c(2,4)]), sampleType = "indepMulti", fixedMargin = "cols")

# BF_h2a_sex_psych_3@bayesFactor$bf # which shows the logBF10
...
BF_h2b_age_psych_2 <- BayesMultiNomial(dataset = df_PSA001_Age_CN_Jasp_new, 
                                     factor = "ageBins", 
                                     observed = "PSA001_CN",
                                     expected = "Census7",
                                     prior = "Census7")
...
BF_h3a_sex <- data.frame(matrix(nrow = 0, ncol = 8))
colnames(BF_h3a_sex) <- c("Observed", "Expected", "Log(BF10)_NonInform", "BF10_NonInform", "BF01_NonInform", 
...
for (ii in seq(length(countries_order_sex))){
  ...
  if (country != "CN"){
    ...
    BF_tmp2 <-  BayesMultiNomial(dataset = df_tmp, 
                               factor = "Sex", observed = country, expected = "CN",prior = "CN")
...
BF_h3a_sex_p <- BF_h3a_sex %>%
  ...
  dplyr::mutate(Contries = factor(Observed, levels = countries_order_sex),
...
Plot figure 4c, 
```{r figure 4c }
fig4c <- ggplot(df_PSA001_valid_merge_sex_ratio_sored, aes(x=Proportion, y=Countries,fill=Sex)) +
  geom_col(alpha = .75)+
...
### Plotting
fig4d <- ggplot(df_PSA001_Age_valid_countryCode_sorted, aes(x=Age, y=Countries, fill=Sex)) +
...
Get the percentage of each age bin and save for Bayesian multinomial test. We found that if the expected proportion contains zero if may cause problem in the calculation, thus, we re-orgainzed the data based on the proportion data of Chinese sample.
...
df_PSA001_age_Jasp <- df_PSA001_Age_valid_countryCode_sorted %>%
  dplyr::select(user_id, Countries, Sex, Age) %>%
  dplyr::mutate(ageBins=cut(Age, 
                            breaks=c(0, 20.5, 30.5, 40.5, 50.5, Inf),  # here the bins are reduced to five.
                            labels=c("11~20","21~30", "31~40", "41~50", ">=51"))) %>%
  dplyr::count(Countries, ageBins) %>%
  dplyr::group_by(Countries) %>%
  dplyr::mutate(freq = (n / sum(n))*100) %>%
  dplyr::ungroup() %>%
...
df_PSA001_age_Jasp2 <- df_PSA001_Age_valid_countryCode_sorted %>%
  dplyr::select(user_id, Countries, Sex, Age) %>%
  dplyr::mutate(ageBins=cut(Age, 
                            breaks=c(0, 17.5, 25.5, 39.5, 59.5, Inf),  # here the bins are reduced to five.
...
df_PSA001_age_Jasp_filled2 <- data.frame(matrix(ncol = ncol(df_PSA001_age_Jasp2), nrow = 0))
colnames(df_PSA001_age_Jasp_filled2) <- colnames(df_PSA001_age_Jasp2)

## below we fill the percentage to 100%
filler_idx3 <- df_PSA001_age_Jasp2 %>%
  dplyr::group_by(Countries) %>%
  dplyr::summarise(sum = sum(percent)) %>%
  dplyr::ungroup() 

for (ii in 1:nrow(filler_idx3)){
  tmp <- df_PSA001_age_Jasp2 %>% 
    dplyr::filter(Countries == filler_idx3$Countries[ii]) %>%
    dplyr::arrange(desc(indx))
  
  if (filler_idx$sum[ii] < 100) {
    for (jj in seq(100 - filler_idx3$sum[ii])){
     tmp$percent[jj] <- tmp$percent[jj] + 1
    }
  }
  df_PSA001_age_Jasp_filled2 <- rbind(df_PSA001_age_Jasp_filled2, tmp)
}
...
df_PSA001_age_Jasp_filled2 <- df_PSA001_age_Jasp_filled2 %>%
  dplyr::arrange(Countries, ageBins) %>%
  dplyr::select(-indx)
...
filler_idx4 <- df_PSA001_age_Jasp_filled2 %>%
  dplyr::group_by(Countries) %>%
  dplyr::summarise(sum = sum(percent)) %>%
  dplyr::ungroup()  
...
df_PSA001_age_Jasp_filled_wide2 <- df_PSA001_age_Jasp_filled2 %>%
  dplyr::select(-c(n, freq)) %>%
  tidyr::pivot_wider(names_from = "Countries", values_from = "percent") %>%
  dplyr::filter(!is.na(ageBins)) %>%
  # fill zero
...
BF_h3b_age_2 <- data.frame(matrix(nrow = 0, ncol = 8))
colnames(BF_h3b_age_2) <- c("Observed", "Expected", "Log(BF10)_NonInform", "BF10_NonInform", "BF01_NonInform", 
                          "Log(BF10)_Inform", "BF10_Inform", "BF01_Inform")

for (ii in seq(length(countries_order_sex))){
  country <- countries_order_sex[ii]
  if (country != "CN"){
    df_tmp <- df_PSA001_age_Jasp_filled_wide2 %>% dplyr::select('ageBins', one_of(country), "CN")
    
    # # de-select the row that both are zero
    # invalid_rows <- c()
    # for (row_tmp in seq(nrow(df_tmp))){
    #   print(paste("current row:", row_tmp))
    #   if (df_tmp[row_tmp, country] + df_tmp[row_tmp, "CN"] ==0) {
    #     print(paste("invalid row:", row_tmp))
    #     invalid_rows <-c(invalid_rows, row_tmp)
    #   }
    # }
    # df_tmp <- df_tmp %>%
    #   dplyr::filter(!row_number() %in% invalid_rows)
    
    BF_tmp <-  BayesMultiNomial(dataset = df_tmp, 
                               factor = "ageBins", observed = country, expected = "CN")
    BF_tmp2 <-  BayesMultiNomial(dataset = df_tmp, 
                               factor = "ageBins", observed = country, expected = "CN", prior = "CN")
    BF_h3b_age_2[ii,] <- c(country, "CN", BF_tmp$BF$LogBF10, BF_tmp$BF$BF10, BF_tmp$BF$BF01,
                         BF_tmp2$BF$LogBF10, BF_tmp2$BF$BF10, BF_tmp2$BF$BF01)
  } 
  
}

countries_order_age_2 <- BF_h3b_age_2 %>% 
  tidyr::drop_na() %>%
  dplyr::mutate(`Log(BF10)_NonInform` = as.numeric(`Log(BF10)_NonInform`)) %>%
  dplyr::arrange(`Log(BF10)_NonInform`) %>%
  dplyr::pull(Observed)

BF_h3b_age_p_2 <- BF_h3b_age_2 %>%
  tidyr::drop_na() %>%
  dplyr::select(1,2,3,6) %>%
  tidyr::pivot_longer(cols = `Log(BF10)_NonInform`:`Log(BF10)_Inform`, 
                      names_to = "Prior",
