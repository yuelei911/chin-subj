...
We decide to use Bayesian multinomial test because it is an appropriate test for our purpose: compare whether the observed distribution (sex, or age bins, etc) is sample from a population distribution (e.g., census data). In frequentist framework, this can be done by $\chi^2$ test (goodness-of-fit).

This can be done using JASP and a short introduction can be found here: Goss-Sampson, M. (2020, May 17). New Bayesian guide. https://doi.org/10.17605/OSF.IO/CKNXM. The function implemented in JASP can be found [here](https://github.com/jasp-stats/jaspFrequencies/blob/d48babf63b14526abaf2c208c8ae769801b6d4db/R/multinomialtestbayesian.R#L384). A more detailed explanation for multinomial test can be found in (Sarafoglou et al., 2020, part 1)
...
Because multinomial distribution has nice conjugate prior, the calculation of multinomial does not involve MCMC. Thus, it is important to be clear about the following questions:
...
The prior of multinomial distribution is a Dirichlet distribution with concentration parameters ($\alpha_1$, $\alpha_2$, ..., $\alpha_k$, where $\alpha > 0$:

$$p(\theta) = p(\theta_1, theta_2, ..., theta_k) = \frac {\Gamma(\sum_{k=1}^k \alpha_k)} {\sum_{k=1}^k\Gamma (\alpha_k)} \prod_{k=1}^k \theta_k^{\alpha_{k}-1}$$

Posterior is also a Dirichlet distribution:

$$p(\theta | x) = \frac {\Gamma(N + \sum_{k=1}^k \alpha_k)} {\sum_{k=1}^k\Gamma (x_k + \alpha_k)} \prod_{k=1}^k \theta_k^{x_k + \alpha_{k}-1}$$

When $H_0$ is that all parameters are equal, i.e., $H_0: \theta = c$, we test the null hypotheses, $H_e$, against the encompassing hypotheses which states that all category proportions are free to vary without any ordinal restriction. 


Using Savage-Dickey Ratio, the BF can be written as:
...
Here prior, ($\alpha_1$, $\alpha_2$, ..., $\alpha_k$) does not need to be equal with $c$. instead, $c$ is a point in the space that defined by Dirichlet distribution. equal $\alpha$ just mean that $c$ is equally possible with other set of $\theta$.

### Algorithm in R

The most essential code in calculating the multnomial distribution in JASP lies in the following code:
...
Here, `alphas` is the prior, usually a vector of 1, `counts` are the observed frequencies.

*Dirichlet* distribution:

$$f(p_1, ..., p_k; \alpha_1, ..., \alpha_k) = \frac{1}{B(\alpha)} \prod_{i=1}^{k} p_i^{\alpha_i - 1}$$

if, $\alpha_1 = \alpha_2 = ... = \alpha_k = 1$, $$f(p_1, ..., p_k; \alpha_1, ..., \alpha_k) = \frac{1}{B(\alpha)}$$

*Beta* function: 

$$B(\alpha) = \frac {\prod_{i = 1}^{k} \Gamma(\alpha_i)}{\Gamma(\sum_{i=1}^k \alpha_i)}$$
Thus, we are using Beta function as the likelihood of multinomial?

This is puzzling: if $\alpha_i$ is not equal to 1, the, $p_i^{\alpha_i -1}$ is not equal to 0, which mean the $\prod_{i=1}^{k} p_i^{\alpha_i - 1}$ should not be eliminated; the R code showed the it used $B(\alpha)$ instead of $\frac{1}{B(\alpha)}$.

So, why we can use $B()$ for calculating the likelihood of multinomial?


Alright, let's put the puzzling $B()$ aside,and test whether the current multinomial test is a special case of contingency table.

(1) let me confirm that the function define here give exactly the same results as in JASP. This is confirmed with sex ratio data and the age bins data. See two JASP files in the folder.

(2) Let me check whether multinomial test is a special case of contingency table. More specifically, it is the third sampling plan, `independent multinomial sampling sheme`. The two data tested above are under the assumption that the column margins are fixed, the BF is calculated by Equation 11 in Jamil et al (2017):

$$BF_{01}^{I} = \frac {D(y_{*.} + \xi_{*.})} {D(\xi_{*.})} \frac {D(y_{.*} + a_{.*})} {D(a_{.*})}  \frac {D(a_{**})} {D(y_{**} + a_{**})} $$

For a n by 2 contingency table, the Bayes factor can be reduced a test for the equality of two proportions, $\theta_1$, $\theta_2$, ... $\theta_n$


using sex ratio data, I found the BF results is quit different: BF =  3.283074 for contingency table, but BF = 41.82 for multinomial test.


Which represent distribution $B(a,b)$, i.e., beta distribution with parameters $a$ and $b$.
...
and the `lgamma` is defined as the log of `gamma`.

$x! $ for non-negative integer x is defined to be $\Gamma (x+1)$

#### "Goodness-of-fit" test for multinomial distribution

Here we used Bayesfactor version of the "goodness-of-fit", the algorithm can be explained as below.

The calculation of Bayes factor here used the Savage-Dickey method, i.e., using the likelihood ratio between posterior and prior of the current dataset.

The log likelihood of the data under prior (a vector of ones):

$$ \sum(log(\Gamma(alphas))) - log(\Gamma(\sum(alphas)))$$

Reference: https://datascience.oneoffcoder.com/dirichlet-multinomial-distribution.html

The likelihood of the data under prior, which is a *Beta* function (why *Beta*?):

$$B(\alpha) = \frac{\prod_{i=1}^{k}\Gamma (\alpha_i)}{\Gamma (\sum_{i=1}^{k} \alpha_i)}$$
Note that *Beta* distribution is closely related to 

also using other function
```{r}
log(dmultinom(x=c(1, 1, 1, 1), prob=c(1/4, 1/4, 1/4, 1/4)))

alphas_tmp <- c(1,1,1,1,1)
lbeta.a_tmp  <- sum(lgamma(alphas_tmp)) - lgamma(sum(alphas_tmp))
exp(lbeta.a_tmp)
```

The log likelihood of data under posterior:

$$\sum(log(\Gamma(alphas + counts))) - log(\Gamma(\sum(alphas + counts))) $$
The likelihood of the data under posterior:

$$\frac{\prod(\Gamma(alphas + counts))}{\Gamma(\sum(alphas + counts))}$$


$$Log(BF_{10}) = \sum(log(\Gamma(alphas + counts))) - log(\Gamma(\sum(alphas + counts))) - (\sum(log(\Gamma(alphas))) - log(\Gamma(\sum(alphas))))$$
where alphas are the prior (default is [1,1,1, ...]), counts is the observed data

$$BF_{10}= \frac {\prod(\Gamma(alphas + counts))/(\Gamma(\sum(alphas + counts)))} {\sum(\Gamma(alphas)) / \Gamma(sum(alphas)))}$$
