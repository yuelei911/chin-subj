...
#### Probability mass function (PMF) of Multinomial distribution
...
Note that *Beta* distribution is closely related to *Dirichlet* distribution:

$$f(p_1, ..., p_k; \alpha_1, ..., \alpha_k) = \frac{1}{B(\alpha)} \prod_{i=1}^{k} p_i^{\alpha_i - 1}$$

if, $\alpha_2 = \alpha_2 = ... =\alpha_k = 1$, $$f(p_1, ..., p_k; \alpha_1, ..., \alpha_k) = \frac{1}{B(\alpha)}$$

also using other function
```{r}
log(dmultinom(x=c(1, 1, 1, 1), prob=c(1/4, 1/4, 1/4, 1/4)))

alphas_tmp <- c(1,1,1,1)
