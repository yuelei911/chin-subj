...
pacman::p_load("tidyverse", "ggh4x","stringr", "patchwork", "BayesFactor")
```
...
```{r test default prior }
### Sex ratio

#### False positive rate

##### Generate data by binomial distribution with p = 0.5 and test with BF
options(scipen=999) 
test_df1 <- data.frame(sex = c("female", "male"),
                       expected = c(50, 50))

test_BF <- data.frame(matrix(nrow = 50, ncol = 4))
colnames(test_BF) <- c("Iter", "1st_prop", "log_BF10", "BF10")


##### Using for loop to test the p = 0.5 ~ 0.75, using 0.05 as interval and repeat the above 
for (ii in seq(1, 49)){
  test_df1$obs <- c(test_df1$expected[1] - ii, test_df1$expected[2] + ii)
  tmp_BF <- BayesMultiNomial(test_df1, factor = 'sex', observed = 'obs', expected = 'expected')
  
  test_BF$Iter[ii] <- ii
  test_BF$`1st_prop`[ii] <- test_df1$expected[1] - ii
...
##### Step 1: generating data using the expected
...
Sim_Power <- function(seed = 12345, sim_N = 1000, prob_sim = 0.5, nil_effect = 0.5){
  set.seed(seed)
...
for (jj in (seq(length(probs)))){
  tmp_power <- Sim_Power(seed = 1000,prob_sim = probs[jj])
...
res_bf_power2 <- res_bf_power
##### Step 2: Testing with BF

#### testing false negative

##### Step 1: generating data using data that has distance with expected.
