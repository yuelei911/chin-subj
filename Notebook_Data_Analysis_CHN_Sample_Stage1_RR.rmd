...
# make sure row sum is not 0, change the zeros in CN data
df_PSA001_age_Jasp_filled_wide_2 <- df_PSA001_age_Jasp_filled_wide  %>% 
  dplyr::mutate(CN = ifelse(CN==0, 1,                    # change 0 t0 1
                            ifelse(CN==55, 53, CN)))     # get to values from the highest row

# non-informative prior
BF_h3b_age <- data.frame(matrix(nrow = 0, ncol = 8))
colnames(BF_h3b_age) <- c("Observed", "Expected", "Log(BF10)_NonInform", "BF10_NonInform", "BF01_NonInform", 
                          "Log(BF10)_Inform", "BF10_Inform", "BF01_Inform")
...
for (ii in seq(length(countries_order_sex))){
  country <- countries_order_sex[ii]
  if (country != "CN"){
    df_tmp <- df_PSA001_age_Jasp_filled_wide_2 %>% dplyr::select('ageBins', one_of(country), "CN")
...
countries_order_age <- BF_h3b_age %>% 
  tidyr::drop_na() %>%
...
fig4b <- ggplot(BF_h3b_age_p, aes( x = `Log(BF10)`, y = Contries, color = Prior )) +
  geom_point() +
  scale_color_grey() +
  xlim(-20, 60) +
  scale_y_discrete(limits=rev) +
