...
We decided to use Bayesian multinomial test because it suits our purpose well: it can be used to test whether observed proportions (sex, or age bins, etc) is sample from the population with certain proprotion values (e.g., census data). In Frequentists' framework, this can be done by $\chi^2$ test (goodness-of-fit).
