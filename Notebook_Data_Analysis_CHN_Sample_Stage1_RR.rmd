...
pacman::p_load("tidyverse", "ggh4x","stringr", "patchwork", "BayesFactor", "gtools")
...
First, the effect size is more complicated: https://github.com/jasp-stats/jasp-issues/issues/1652
...
prob_mult <- c(0.21, 0.08, 0.23, 0.29, 0.19)

sim_tmp <- rmultinom(n=10, size=100, prob=prob_mult)

expected <- prob_mult*100

sim_data <- data.frame(ageBins = paste("Bin", seq(5), sep = ""),
                       expected = expected,
                       obs = sim_tmp[,1])


tmp_BF_mult <- BayesMultiNomial(dataset = sim_data, 
                                factor = "ageBins", 
                                observed = "obs",
                                expected = "expected")
...
seed = 12345
sim_N = 1000
prob_sim = c(0.21, 0.08, 0.23, 0.29, 0.19)
nil_effect = c(0.21, 0.08, 0.23, 0.29, 0.19)

c(0.21, 0.08, 0.23, 0.29, 0.19)

# bin 1 increase 0.01
## c(0.22, 0.07, 0.23, 0.29, 0.19)
## c(0.22, 0.08, 0.22, 0.29, 0.19)
## c(0.22, 0.08, 0.23, 0.28, 0.19)
## c(0.22, 0.08, 0.23, 0.29, 0.18)

tmp_prob <- data.frame(rdirichlet(1000, c(2,2,2,2,2)))
colnames(tmp_prob) <- paste0("bin", seq(5))

sim_data <-  data.frame(matrix(nrow = 1000, ncol = 3))
  colnames(test_BF) <- c("Iter", "distance", "BF10")

rm("sim_data")
for (ii in seq(1000)){
  tmp_data <-data.frame(rmultinom(n=1, size=100, prob = tmp_prob[ii, ]))  %>%
...
for (jj in seq(1000)){
    test_df <- sim_data[jj, ] %>%
      pivot_longer(cols = bin1:bin5,
                   names_to = "ageBins",
                   values_to = 'obs')
    
    test_df$expected = nil_effect*100
    # test_df <- data.frame(ageBins = paste("bin", seq(5), sep = ""),
    #                       expected = nil_effect*100,
    #                       obs = sim_data[,ii])
    ...
    # test_BF$`1st_prop`[ii] <- test_df$expected[1] - ii
    # test_BF$`log_BF10`[ii] <- tmp_BF$BF$LogBF10
    test_BF$`BF10`[jj] <- tmp_BF$BF$BF10
    
}

sum(test_BF$BF10 >=3)/1000
...
Sim_Power_mult <- function(seed = 12345, 
  ...
  sim_data <- rmultinom(n=sim_N, size=100, prob=prob_sim)
  ...
  if (sum(prob_sim == nil_effect)==length(prob_sim)) {
    BF_power$Prob <- paste0(prob_sim, collapse = ';')
    BF_power$Category <- "Evidence_for_Null"
    BF_power$BF_1 <- sum(test_BF$BF10 < 1)/1000 
    BF_power$BF_3 <- sum(test_BF$BF10 <= 1/3)/1000 
    BF_power$BF_6 <- sum(test_BF$BF10 <= 1/ 6)/1000 
...
tmp_bf_pow_mult <- Sim_Power_mult()
