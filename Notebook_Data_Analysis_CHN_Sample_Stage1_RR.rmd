...
(Lines 191 to 738 of the code, that's our simulations, this part of the code takes a long time to run. Therefore, it is not shown their running results in our html file(Notebook_Data_Analysis_CHN_Sample_Stage1_RR.html), see Rmarkdown for details(Notebook_Data_Analysis_CHN_Sample_Stage1_RR.RMD)
...
Thank Mengzheng for writing it in parallel.

```{r foreach package,eval=FALSE}
...
The simulation will use N as 1200 so that it will reveal the senstivity of the sample size that resulted from sex ratio data.
```{r sensitivity analysis by mean age}
## First, we get a reasonable  mean age range by using PSA 001's data

# We used the bootstrap method to get the mean and 95% CI of the age

# We we also used the bootstrap method to get the mean of SD and the distribution of SD
...
sim_sd <- df_PSA001_CN_SD
meanAges <- c(10, 15, 20, df_PSA001_CN_M, 25, 30, 35, 40, 45, 50, 55, 60)

rm('sim_MNtest')
sim_MNtest <- expand.grid(MeanAge1 = meanAges,
                          age_diff = sim_age_diff,
                          SD = sim_sd)
sim_MNtest$BF10 <- NA
sim_MNtest$LogBF10 <- NA
for (iter in seq(nrow(sim_MNtest))) {
  tmp_BF <- Sim_Sens_mult(Mean1=sim_MNtest$MeanAge1[iter], 
                          diff_age=sim_MNtest$age_diff[iter],
                          SD1=sim_MNtest$SD[iter], 
                          SD2=sim_MNtest$SD[iter], 
                          sampe_N=1200)

  sim_MNtest$BF10[iter] <- tmp_BF$BF$BF10
  sim_MNtest$LogBF10[iter] <- tmp_BF$BF$LogBF10

}

figR2_1 <- sim_MNtest %>% 
  ggplot2::ggplot(aes(x = age_diff, y = LogBF10, color = as.factor(MeanAge1))) + 
  ggplot2::geom_point() +
...
Sim_Sens_mult_alt <- function(Mean1, diff_age, SD1, SD2, sampe_N=1200, sim_N=1000){
  ...
  for (iter in seq(sim_N)){
    ...
    ageData1 <- round(rtruncnorm(n=sampe_N, a=0, b =100, mean = Mean1, sd = SD1))
    ageData2 <- round(rtruncnorm(n=sampe_N, a=0, b =100, mean = Mean2, sd = SD2))
...
### test the function non-parallel
startTime <- Sys.time()
rm('sim_MNtest_alt')
for (iter in seq(nrow(sim_age_param[1:4, ]))) {
  
    tmp_sens <- Sim_Sens_mult_alt(Mean1=sim_age_param$MeanAge1[iter], 
                                  diff_age=sim_age_param$age_diff[iter],
                                  SD1=df_PSA001_CN_SD, 
                                  SD2=df_PSA001_CN_SD, 
                                  sampe_N=1200,
                                  sim_N=1000)
    
    if (!exists('sim_MNtest_alt')){
      sim_MNtest_alt <- tmp_sens
    } else{
      sim_MNtest_alt <- rbind(sim_MNtest_alt, tmp_sens)
    }
}

endTime <- Sys.time()
print(endTime - startTime)   # 1.26 min for 4 iterates
...
fig3b <- ggplot(data= df_census7_age_new, aes(x=ageBins, y=ifelse(sex=="male", -percent, percent), fill=sex)) +
  ...
  geom_line(data = df_PSA001_Age_CN, aes(x=ageBins_pyr,
                                      y = ifelse(Sex == "male_PSA", -percent/3.5, percent/3.5),
                                      group=Sex,color="PSA001"), size=1, inherit.aes = FALSE) +
  scale_y_continuous(limits = c(-15,15),sec.axis = sec_axis(~.*3.5, name = "Proportion (PSA001)")) +
  coord_flip() +
...
df_PSA001_valid_merge <- merge(df_PSA001_valid_sex, df_regionCode, by.x = "Countries", by.y = "Jones_country")
...
Func_BFpairs <- function(df, countries_order, prop_name){
  ...
  for (ii in seq(length(countries_order))){
    ...
    if (country != "CHN"){
      ...
      BF_mult_tmp[ii,] <- c(country, "CHN", BF_tmp$BF$LogBF10, BF_tmp$BF$BF10, BF_tmp$BF$BF01)
    } 
  }
...
df_PSA001_Age_valid_countryCode <- merge(df_PSA001_Age_valid, df_regionCode, by.x = "Countries", by.y = "Jones_country") %>%
...
df_PSA001_age_Jasp1 <- df_PSA001_age_Jasp1 %>%
  # fill zero
  dplyr::mutate(CHN = ifelse(CHN==0, 1,
                            ifelse(CHN==38, 37, 
                                   ifelse(CHN  == 55, 54, CHN))))
