...
fig4d_supp <- ggplot(df_PSA001_age_Jasp1_long, aes(x=Proportion, y=Countries, fill=ageBins)) +
  geom_col(alpha = .75)+
  labs(y="Countries") + 
...
fig4b_supp <- ggplot(BF_h3b_age_p_1, aes( x = `Log(BF10)`, y = Contries, color = Prior )) +
  geom_point() +
  scale_color_grey() +
  xlim(-10, 60) +
  scale_y_discrete(limits=rev) +
  ...
  theme_bw() +
        theme(panel.grid.major = element_blank(),
              ...
              panel.spacing = unit(3, "lines"))
#fig4b_supp
p_BF_supp <- (fig4a + fig4b_supp) /
   (fig4c + fig4d_supp) +
  plot_annotation(tag_levels = 'A')  & 
  theme(plot.tag = element_text(size = 18, face = "bold"))

ggsave("Fig4_withBF_supp.pdf", p_BF_supp, device = "pdf", width=18, height = 18)
p_BF_supp
```
