...
```{r define a func for age bins of census data}

Func_ageBin_census <- function(){
...
df_PSA001_Age_CN <- df_PSA001 %>%
  ...
  dplyr::mutate(ageBins_pyr = cut(Age,
                                  breaks=c(-Inf, 4.5, 9.5, 14.5, 19.5, 24.5, 29.5, 34.5, 39.5, 44.5,
                                            49.5, 54.5,59.5,64.5,69.5,74.5,79.5,84.5,89.5,94.5, Inf),
...
df_census7_age94 <- df_census7 %>%
  ...
  dplyr::slice(c(9:13,16:20,23:27,30:34,37:41,
                 44:48,50:54,57:61,64:68,71:75,
                 78:82,85:89,92:96,99:103,106:110,
                113:117, 120:124,127:131,134:138)) %>%
  dplyr::rename(ageBins=1,
                male=2,
                female=3) %>% 
  dplyr::mutate(ageBins=as.numeric(ageBins),
         male=as.numeric(male),
         female=as.numeric(female)) %>% 
  dplyr::mutate(male=round(male/1409778724*100,2),
                          female=round(female/1409778724*100,2)) %>% 
