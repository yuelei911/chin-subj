...
```{r test default prior for sex ratio, message=TRUE, warning=FALSE}
...
probs <- seq(0.5, 0.7, 0.01) # create a seqenece from 0.5 to 0.7, with 0.01 step size.

# iterate each of theses probabilities and save the percentage of BF10 >=3, BF10 >=60
rm(res_bf_power_binom) # remove variable with name "res_bf_power_binom" to avoid error
for (jj in (seq(length(probs)))){
  tmp_power <- Sim_Power_binormial(seed = 1000, prob_sim = probs[jj])
  if (exists("res_bf_power_binom")){
    res_bf_power_binom <- rbind(res_bf_power_binom, tmp_power)
  } else {
    res_bf_power_binom <- tmp_power
  }
}
