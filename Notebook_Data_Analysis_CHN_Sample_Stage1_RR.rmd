...
```{r define a function for calculating BF for multiple countries}
...
BF_h3a_sex_p <- Func_BFpairs(df = df_PSA001_valid_merge_sex_ratio_sored_wide,
...
```{r define a func for proportion of age bins}

Func_Age_Prop <- function(df, break_names, label_names){
...
df_PSA001_age_Jasp0 <- Func_Age_Prop(df = df_PSA001_Age_valid_countryCode_sorted,
                                     break_names = c(0, 10.5, 20.5, 30.5, 40.5, 50.5, 60.5, Inf),
                                     label_names = c("0~10","11~20","21~30", "31~40", "41~50", "51~60", ">=61"))

df_PSA001_age_Jasp0 <- df_PSA001_age_Jasp0 %>%
  # fill zero
  dplyr::mutate(CN = ifelse(CN==0, 1, 
                            ifelse(CN==38, 37, 
                                   ifelse(CN == 55, 54, CN))))

# age bin similar to census data (truncated)
df_PSA001_age_Jasp1 <- Func_Age_Prop(df = df_PSA001_Age_valid_countryCode_sorted,
                                     break_names = c(0, 20.5, 30.5, 40.5, 50.5, Inf),
                                     label_names = c("11~20","21~30", "31~40", "41~50", ">=51"))
...
BF_h3b_age_p_0 <- Func_BFpairs(df = df_PSA001_age_Jasp0, 
                             countries_order = countries_order_sex,
                             prop_name = "ageBins")
