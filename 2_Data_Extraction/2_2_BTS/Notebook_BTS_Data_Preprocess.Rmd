...
```{r}
## load Region
load("BTS_region.Rdata") ## include 6 projects

load("BTS_dupli.Rdata") ## A total of 27 projects were included
```


Convert data format from data.frame to list
...
names(BTS[[16]])[names(BTS[[16]]) == "Country_now"] <- "Country"  ## BTS_Klein 2018 Slate1

names(BTS[[17]])[names(BTS[[17]]) == "Country_now"] <- "Country"  ## BTS_Klein 2018 Slate2

names(BTS[[28]])[names(BTS[[28]]) == "Country_now"] <- "Country"  ## BTS_Wang 2021

names(BTS[[25]])[names(BTS[[25]]) == "Country_ID"]  <-  "Country" ## BTS_Tybur 2016


BTS[[28]]$Country <- trimws(sub(".*-(\\s*)", "", BTS[[28]]$Country))  ## BTS_Wang 2021


## process abnormal/informal Countrycode
...
BTS[[15]] <- BTS[[15]] %>% 
   dplyr::mutate(Country = case_when(Country %in% c("Canada_EN","Canada_FR") ~ "Canada",
                                    ...
                                    Country %in% "Hong Kong" ~ "Hong Kong SAR China",
...
Unify the column name of country code
```{r}
...
Taiwan, Hong Kong and Macao are all incorporated into China's data.
```{r}
BTS <- lapply(BTS,function(df){
  
  df <- df %>% 
    dplyr::mutate(ISO3 = case_when(ISO3 %in% c("HKG","TWN","MAC") ~ "CHN",
                                   TRUE ~ ISO3),
                  ISO2 = case_when(ISO2 %in% c("HK","TW","MO") ~ "CN",
                                   TRUE ~ ISO2)) %>% 
    dplyr::select(-c(Country,Countrycode,Countryfullname))
  return(df)
...
for (i in seq_along(BTS)) {
  ...
  if("Education" %in% names(BTS[[i]])){
    ...
    BTS_edu <- BTS[[i]] %>% 
      dplyr::select(Article_Id,ISO3,Education)
...
Convert edu data into two cutoff
```{r}
## BTS_Arechar_et_al_2023
...
# Load BTS doing data
```{r}
BTS_coding <- readxl::read_xlsx("Chin_Subj_Proofreading_BTS_Target_Population.xlsx")
```


# Save Rdata
```{r}
save(BTS, WEIRD, BTS_edu_list, Countrycodelist_iso2_iso3, 
     BTS_region, BTS_coding, file = "BTS.Rdata")
```
