...
BTS_Overview2 <- BTS_Overview %>% 
  ...
  dplyr::mutate(Proportion = round(n/sum(n)*100,2),
                ...
                Proportion_adjust = Proportion / Proportion_sum,
                Proportion_sum_adjust = sum(Proportion_adjust),
                Site = "BTS")
```
...
Sim_Journal_age_five3 <- Sim_Journal_age_five2 %>% 
  ...
  dplyr::summarise(sim1 = round(sum(sim1,na.rm = TRUE),2)) %>% 
  dplyr::ungroup() %>% 
...
BTS_age_general_CHN <- BTS_age %>% 
  ...
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
                Proportion_sum = sum(Proportion),
                Proportion_adjust = Proportion / Proportion_sum * 100,
                Proportion_final_sum = sum(Proportion_adjust),
...
```{r}
China_Central <- c("山西","安徽","江西","河南","湖北","湖南")

China_Eastern <- c("北京","天津","河北","上海","江苏","浙江","福建","山东","广东","海南")

China_Western <- c("内蒙古","广西","重庆","四川","贵州","云南","西藏","陕西","甘肃","青海","宁夏","新疆")

China_Northeastern <- c("辽宁","吉林","黑龙江")

China_four_regions <- tibble(Region = c(China_Central, China_Eastern, 
                                          China_Western, China_Northeastern),
                             Region_four = c(rep("China_Central", 6), rep("China_Eastern", 10),
                                        rep("China_Western", 12),rep("China_Northeastern", 3)))
```
...
convert into four regions
```{r}
Journal_four_regions <- merge(Journal_region_general_tab2,China_four_regions,
                              by = "Region") %>% 
  dplyr::select(1,2,6) %>% 
...
## four regions
Census7_four_region <- merge(Census7_region,China_four_regions,by = "Region") %>%
  dplyr::select(1,2,6) %>% 
  dplyr::group_by(Region_four) %>% 
...
## four regions
CFPS2018_four_region <- merge(CFPS2018_region,China_four_regions,by = "Region") %>%
  dplyr::select(1,2,6) %>% 
  dplyr::group_by(Region_four) %>% 
...
```{r}
H2_region <- full_join(Journal_four_regions,Census7_four_region,by = "Region_four") %>% 
  dplyr::full_join(CFPS2018_four_region,by = "Region_four") %>% 
...
## Visualization
H2_four_region <- ggplot(data = H2_region, aes(x = Data_source, y = Proportion, fill = Region_four)) +
  geom_col(position = "stack")+
  labs(fill = "Region")+
  scale_x_discrete(labels =c( "Census7_Prop" = "Census7",
...
ggsave("H2_four_region .pdf",H2_four_region, device = "pdf", width= 12, height = 9)
