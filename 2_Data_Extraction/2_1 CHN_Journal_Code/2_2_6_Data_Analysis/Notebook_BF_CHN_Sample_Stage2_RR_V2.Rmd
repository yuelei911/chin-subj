...
load("D://chin-subj//2_Data_Extraction//2_2_BTS//BTS_coding.Rdata")
...
Journal_simple_info_articles <- df_stage2_Journal_Code2 %>% 
  dplyr::select(Article_IDs,Remark1,Remark2,Remark3,Subjects_Recruitment_Area) %>% 
  dplyr::filter(!Article_IDs %in% Journal_articles_num$Article_IDs) %>% 
  dplyr::distinct(Article_IDs,.keep_all = TRUE)   ## 这里20082248排除错误，后面记得处理
...
# sum(Journal_sample_size$Sample_Size,na.rm = TRUE)  ## 538,552
...
Journal_special_target_population2 <- readxl::read_xlsx("Journal_special_target_population.xlsx") %>% 
  dplyr::select(20,22,7,19,23,24,28) %>% 
...
Journal_children_adolenscent_TP <- Journal_special_target_population2 %>% 
  dplyr::filter(grepl("儿童|幼儿|青少年|初中生|高中生|小学生|中学生|中职",Recode_target_population))
```
...
Target_population_general_gender <- Journal_gender2 %>% 
  ...
  dplyr::summarise(Male_Num = sum(Male_Num,na.rm = TRUE),
                   ...
                   Male_Proportion = round((Male_Num/(Male_Num + Female_Num))*100,2),
                   Female_Proportion = round((Female_Num/(Male_Num + Female_Num))*100,2),
...
Target_population_special_sampletype23_gender <- Journal_gender2 %>% 
  ...
  dplyr::summarise(Male_Num = sum(Male_Num,na.rm = TRUE),
                   ...
                   Data_Source = "Target_population_special23")

Journal_gender_different_categories <- bind_rows(Target_population_general_gender,
                                                 Target_population_special_gender,
                                                 Target_population_special_sampletype23_gender) 
```
...
H1_gender_general <- BTS_gender_CHN_general %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(273804,234047),
                 Proportion = c(54,46),Data_Source = "Journal")   ## Here, enter data manually.  ## notice this proportion
...
observed1 <- H1_gender_bayes_general$`BTS Chinese`
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_gender_general$parameter,thresh = 6)
...
In this part, I will analysis gender based on the special target population.

first, I process Journal data

```{r}
Journal_college_TP_articleIDs <- Journal_college_TP %>% 
  dplyr::distinct(Article_IDs) %>% 
  dplyr::pull(Article_IDs)

Journal_gender_special_college <- Journal_gender2 %>% 
  dplyr::filter(Article_IDs %in% Journal_college_TP_articleIDs)


## combine two data source
## I found there is no data of BTS on college in special target population.
```
...
Sim_Journal_age_five3 <- Sim_Journal_age_five2 %>% 
  ...
  dplyr::summarise(sim1 = round(sum(sim1,na.rm = TRUE),4)) %>% 
  dplyr::ungroup() %>% 
...
## fill to percent 100
Sim_Journal_age_five3 <- Sim_Journal_age_five3 %>% 
  dplyr::mutate(Floorsim1 = floor(sim1),
                index = sim1 - Floorsim1,
                Site = "Journal")

Sim_Journal_age_five3_idx <- Sim_Journal_age_five3 %>% 
  dplyr::group_by(Site) %>% 
  dplyr::summarise(sum=sum(Floorsim1))


Sim_Journal_age_five3_fill <- data.frame(matrix(ncol = ncol(Sim_Journal_age_five3),nrow = 0))
colnames(Sim_Journal_age_five3_fill) <- colnames(Sim_Journal_age_five3)


for (i in 1:nrow(Sim_Journal_age_five3_idx)) {
  
  df_sim1 <- Sim_Journal_age_five3 %>% 
    dplyr::filter(Site == Sim_Journal_age_five3_idx$Site[i]) %>% 
    dplyr::arrange(desc(index))
  
  if(Sim_Journal_age_five3_idx$sum[i] < 357734){
    
    for (j in seq(357734 - Sim_Journal_age_five3_idx$sum[i])) {
      
      df_sim1$Floorsim1[j] <- df_sim1$Floorsim1[j] + 1
      
    }
    
    Sim_Journal_age_five3_fill <- rbind(Sim_Journal_age_five3_fill,df_sim1)
    
  }
  
  return(Sim_Journal_age_five3_fill)
  
}


## Calculate the sample difference between the coded data and the simulated data
sum(Journal_age2$Sample_Size)-sum(Sim_Journal_age_five3_fill$Floorsim1,na.rm = TRUE) 
```
...
## fill to percent 100
Sim_Journal_age_interval103 <- Sim_Journal_age_interval103 %>% 
  dplyr::mutate(Floorsim1 = floor(sim1),
                index = sim1 - Floorsim1,
                Site = "Journal")

Sim_Journal_age_interval103_idx <- Sim_Journal_age_interval103 %>% 
  dplyr::group_by(Site) %>% 
  dplyr::summarise(sum=sum(Floorsim1))


Sim_Journal_age_interval103_fill <- data.frame(matrix(ncol = ncol(Sim_Journal_age_interval103),nrow = 0))
colnames(Sim_Journal_age_interval103_fill) <- colnames(Sim_Journal_age_interval103)


for (i in 1:nrow(Sim_Journal_age_interval103_idx)) {
  
  df_sim1 <- Sim_Journal_age_interval103 %>% 
    dplyr::filter(Site == Sim_Journal_age_interval103_idx$Site[i]) %>% 
    dplyr::arrange(desc(index))
  
  if(Sim_Journal_age_interval103_idx$sum[i] < 357734){
    
    for (j in seq(357734 - Sim_Journal_age_interval103_idx$sum[i])) {
      
      df_sim1$Floorsim1[j] <- df_sim1$Floorsim1[j] + 1
      
    }
    
    Sim_Journal_age_interval103_fill <- rbind(Sim_Journal_age_interval103_fill,df_sim1)
    
  }
  
  return(Sim_Journal_age_interval103_fill)
...
## Calculate the sample difference between the coded data and the simulated data
sum(Journal_age2$Sample_Size)-sum(Sim_Journal_age_interval103_fill$Floorsim1,na.rm = TRUE)
```
...
## Calculate the sample difference between the coded data and the simulated data
sum(Journal_age_general$Sample_Size)-sum(Sim_Journal_age_five3_general$sim1,na.rm = TRUE) 
```
...
Sim_Journal_age_five3_general_prop <- Sim_Journal_age_five3_general %>% 
  dplyr::mutate(Proportion = round((sim1/sum(sim1))*100,4),
                Data_Source = "Journal")

sum(Sim_Journal_age_five3_general_prop$Proportion)  ## sum 100.0002


## fill to percent 100
Sim_Journal_age_five3_general_prop <- Sim_Journal_age_five3_general_prop %>% 
  dplyr::mutate(FloorPro = floor(Proportion),
                index = Proportion - FloorPro)

Sim_Journal_age_five3_general_prop_index <- Sim_Journal_age_five3_general_prop %>% 
  dplyr::group_by(Data_Source) %>% 
  dplyr::summarise(sum = sum(FloorPro)) %>% 
  dplyr::ungroup()


Sim_Journal_age_five3_general_prop_fill <- data.frame(matrix(ncol = ncol(Sim_Journal_age_five3_general_prop),
                                                             nrow = 0))
colnames(Sim_Journal_age_five3_general_prop_fill) <- colnames(Sim_Journal_age_five3_general_prop)

rm(df_sim1)

for (i in 1:nrow(Sim_Journal_age_five3_general_prop_index)) {
  
  df_sim1 <- Sim_Journal_age_five3_general_prop %>% 
    dplyr::filter(Data_Source == Sim_Journal_age_five3_general_prop_index$Data_Source[i]) %>% 
    dplyr::arrange(desc(index))
  
  if(Sim_Journal_age_five3_general_prop_index$sum[i] < 100){
    
    for(j in seq(100 - Sim_Journal_age_five3_general_prop_index$sum[i])){
      
      df_sim1$FloorPro[j] <- df_sim1$FloorPro[j] + 1
      
    }
    
    Sim_Journal_age_five3_general_prop_fill <- rbind(Sim_Journal_age_five3_general_prop_fill,df_sim1)
    
  }
  
  return(Sim_Journal_age_five3_general_prop_fill)
  
}

sum(Sim_Journal_age_five3_general_prop_fill$FloorPro) ## 100
...
sum(Sim_Journal_age_PsyStages3_general_prop$Proportion)
```
...
sum(Sim_Journal_age_interval103_general_prop$Proportion) ## 100
...
#                 index = Proportion - FloorPro)
# 
# Sim_Journal_age_interval103_general_prop_index <- Sim_Journal_age_interval103_general_prop %>% 
#   dplyr::group_by(Data_Source) %>% 
#   dplyr::summarise(sum=sum(FloorPro)) %>% 
#   dplyr::ungroup()
# 
# Sim_Journal_age_interval103_general_prop_fill <- data.frame(matrix(ncol = ncol(Sim_Journal_age_interval103_general_prop),nrow = 0))
# colnames(Sim_Journal_age_interval103_general_prop_fill) <- colnames(Sim_Journal_age_interval103_general_prop)
# 
# for (i in 1:nrow(Sim_Journal_age_interval103_general_prop_index)) {
#   
#   df_sim_interval103_general <- Sim_Journal_age_interval103_general_prop %>% 
#     dplyr::filter(Data_Source == Sim_Journal_age_interval103_general_prop_index$Data_Source[i]) %>% 
#     dplyr::arrange(desc(index))
#   
#   if(Sim_Journal_age_interval103_general_prop_index$sum[i] < 100){
#     
#     for (j in seq(100 - Sim_Journal_age_interval103_general_prop_index$sum[i])) {
#       
#         df_sim_interval103_general$FloorPro[j] <-    df_sim_interval103_general$FloorPro[j] + 1
#       
#     }
#     
#     Sim_Journal_age_interval103_general_prop_fill <- rbind(Sim_Journal_age_interval103_general_prop_fill,
#                                                      df_sim_interval103_general)
#     
#   }
#   
#   return(Sim_Journal_age_interval103_general_prop_fill)
# 
# }
# 
# sum(Sim_Journal_age_interval103_general_prop_fill$FloorPro)
```


sepcial target population
...
# str(BTS1)## 使用BTS1是因为部分数据年龄有问题，所以筛选，后续有多处是这样，数据再次清洗后请注意更换

BTS_age_list <- list()

for(i in seq_along(BTS1)){
  
  BTS_name <- names(BTS1)[i]
  
  if("Age" %in% names(BTS1[[i]])){
...
considering the different target population, I process all data firstly.
...
sum(BTS_age_general_CHN$Proportion) ## 99.9999

# ## fill to percent 100
# BTS_age_general_CHN <- BTS_age_general_CHN %>% 
#   dplyr::mutate(FloorPro = floor(Proportion),
#                 index = Proportion - FloorPro)
# 
# BTS_age_general_CHN_index <- BTS_age_general_CHN %>% 
#   dplyr::group_by(Data_Source) %>% 
#   dplyr::summarise(sum = sum(FloorPro)) %>% 
#   dplyr::ungroup()
# 
# 
# BTS_age_general_CHN_fill <- data.frame(matrix(ncol = ncol(BTS_age_general_CHN),
#                                                              nrow = 0))
# colnames(BTS_age_general_CHN_fill) <- colnames(BTS_age_general_CHN)
# 
# rm(df_sim1)
# 
# for (i in 1:nrow(BTS_age_general_CHN_index)) {
#   
#   df_sim1 <- BTS_age_general_CHN %>% 
#     dplyr::filter(Data_Source == BTS_age_general_CHN_index$Data_Source[i]) %>% 
#     dplyr::arrange(desc(index))
#   
#   if(BTS_age_general_CHN_index$sum[i] < 100){
#     
#     for(j in seq(100 - BTS_age_general_CHN_index$sum[i])){
#       
#       df_sim1$FloorPro[j] <- df_sim1$FloorPro[j] + 1
#       
#     }
#     
#     BTS_age_general_CHN_fill <- rbind(BTS_age_general_CHN_fill,df_sim1)
#     
#   }
#   
#   return(BTS_age_general_CHN_fill)
#   
# }
# # sum(BTS_age_general_CHN_fill$FloorPro) ## 100


## combine 2 dataframe
H1_age_general <- Sim_Journal_age_five3_general_prop %>%
  dplyr::rename(n = 2) %>%
  dplyr::bind_rows(BTS_age_general_CHN) ## 这里缺少>=95
...
# sum(BTS_age_PsyStages_CHN_general$Proportion) ## 100
...
# sum(BTS_age_interval10_CHN_general$Proportion) ## 100
```
...
```{r}
## convert data format 
H1_PsyStages_bayes_general <- Sim_Journal_age_PsyStages3_general_prop %>% 
  dplyr::mutate(Data_Source = Proportion) %>%  
  dplyr::select(-c(sim1,Proportion)) %>% 
  dplyr::rename(Journal = 2) %>% 
  dplyr::left_join(BTS_age_PsyStages_CHN_general %>%
                     mutate(Data_Source = n) %>%
                     rename(`BTS CHN age` = Data_Source) %>% 
...
## calculate Chi-square
observed1 <- H1_PsyStages_bayes_general$`BTS CHN age`
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_PsyStages_general$parameter,thresh = 6)
...
```{r}
## convert data format 
H1_interval10_bayes_general <- Sim_Journal_age_interval103_general_prop %>% 
  dplyr::mutate(Data_Source = Proportion) %>%  
  dplyr::select(-c(sim1,Proportion)) %>% 
  dplyr::rename(Journal = 2) %>% 
...
## calculate Chi-square
observed1 <- H1_interval10_bayes_general$`BTS CHN age`
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_interval10_general$parameter,thresh = 6)

BF1 <- froot_bf(x = H1_Chi_interval10_general$statistic, 
                lambda = lambda, 
                df = H1_Chi_interval10_general$parameter)
...
## analysis edu base on terget population
## general population
Journal_edu_general_article <- Journal_key_variable %>% 
  dplyr::select(Article_IDs,Target_Population_N) %>% 
  dplyr::distinct(Article_IDs,Target_Population_N) %>%   ## 20219132目标人群需要重新处理
  dplyr::filter(Target_Population_N != 1)
...
Journal_edu2_tab <- Journal_edu_general %>% 
...
#   dplyr::select(1:2) %>% 
  ...
  tidyr::pivot_longer(cols = c(College_and_above,Below_college),
                      ...
                      values_to = "Num") %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "Journal")

# sum(Journal_college2$Proportion)  ## 100
...
Journal_high_school2  <- #Journal_edu2_year_simul_sum %>% 
  ...
  tidyr::pivot_longer(cols = c(High_school_and_above,Below_high_school),
                      names_to = "Educational_attainment",
                      values_to = "Num") %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,2),
                Data_source = "Journal")  

# sum(Journal_high_school2$Proportion) ## 100
```
...
for(i in seq_along(BTS1)){
  ...
  if("Education" %in% names(BTS1[[i]])){
    
    BTS_edu <- BTS1[[i]] %>% 
      dplyr::filter(!is.na(Education) & !is.numeric(Education) & !is.na(ISO3)) %>%
      dplyr::select(Article_Id,Education,Target_Population,ISO3) %>%
      dplyr::mutate(Data_Source = BTS_name)
...
## merge data.frame in list
BTS_edu <-  do.call(rbind,BTS_edu_list)
# 
# 
# bts1_edu <- data.frame(table(BTS_edu_CHN$Education))
# writexl::write_xlsx(bts1_edu,"bts1_edu.xlsx")
```
...
```{r}
BTS_edu <- BTS_edu %>% 
  dplyr::mutate(College_cate = ifelse(Education %in% c("Bachelor","degree","Doctoral","doctorate",
                                                              "Doctorate","four year degree","Four year degree",
                                                              "Graduate","Masters","professional degree",
                                                          "Professional degree","some college","Some college",
                                                          "tertiary","two year degree","Two year degree",
                                                          "University Masters","University MBA","University PhD",
                                                          "University Undergraduate"),"College_and_above",
                                      ifelse(Education %in% c("Elementary school","high school","High school",
                                                          "I don't have any","less than high school","Less than high school",
                                                          "no formal","No formal education","primary",
                                                          "Primary ed.","Primary school","Primary school only",
                                                          "Primary school through Secondary school","secondary","Secondary ed.",
                                                          "Secondary school") ,"Below_college", NA)),
                Highschool_cate = ifelse(Education %in% c("Bachelor","degree","Doctoral","doctorate",
                                                              "Doctorate","four year degree","Four year degree",
                                                              "Graduate","Masters","professional degree",
                                                          "Professional degree","some college","Some college",
                                                          "tertiary","two year degree","Two year degree",
                                                          "University Masters","University MBA","University PhD",
                                                          "University Undergraduate","high school","High school"),"High_school_and_above",
                                         ifelse(Education %in% c("Elementary school","I don't have any","less than high school",
                                                          "Less than high school","no formal","No formal education",
                                                          "primary","Primary ed.","Primary school",
                                                          "Primary school only"), "Below_high_school",NA)))
...
## calculate
BTS_college <- BTS_edu_CHN %>%
  dplyr::group_by(College_cate) %>%
  dplyr::summarise(n=n()) %>% ## there are 995 NA values
  dplyr::filter(!is.na(College_cate)) %>% 
  dplyr::rename(Educational_attainment = 1, Num = 2) %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "BTS Chinese")

 # sum(BTS_college$Proportion) ## 100
  
  
BTS_highschool <- BTS_edu_CHN %>%
  dplyr::group_by(Highschool_cate) %>%
  dplyr::summarise(n=n()) %>% ## there are 1647 NA values
  dplyr::filter(!is.na(Highschool_cate)) %>% 
  dplyr::rename(Educational_attainment = 1, Num = 2) %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "BTS Chinese")

# sum(BTS_highschool$Proportion) ## 100


## Combine data
H1_edu_college <- rbind(Journal_college2,BTS_college)

H1_edu_highschool <- rbind(Journal_high_school2,BTS_highschool)
```
...
H1_edu_college_bayes  <- H1_edu_college %>% 
  dplyr::filter(Data_source == "BTS Chinese") %>% 
  dplyr::select(Educational_attainment, `BTS Chinese` = Num) %>% 
  dplyr::left_join(H1_edu_college %>% 
                     filter(Data_source == "Journal") %>% 
                     select(Educational_attainment, Journal = Proportion),
                   by = "Educational_attainment") %>% 
  dplyr::mutate(Journal = Journal*0.01)

## calculate Chi-square
observed1 <- H1_edu_college_bayes$`BTS Chinese`
expected1 <- H1_edu_college_bayes$Journal

H1_Chi_edu_college <- chisq.test(x = observed1, p = expected1)
H1_Chi_edu_college


## calculate BF
lambda <- UMPBT(df = H1_Chi_edu_college$parameter,thresh = 6)

BF1 <- froot_bf(x = H1_Chi_edu_college$statistic, lambda = lambda, df = H1_Chi_edu_college$parameter)

H1_BF_edu_college  <- log(BF1)
...
## calculate Chi-square
observed1 <- H1_edu_highschool_bayes$`BTS Chinese`
expected1 <- H1_edu_highschool_bayes$Journal

H1_Chi_edu_highschool <- chisq.test(x = observed1, p = expected1)
H1_Chi_edu_highschool 


## calculate BF
lambda <- UMPBT(df = H1_Chi_edu_highschool$parameter,thresh = 6)

BF1 <- froot_bf(x = H1_Chi_edu_highschool$statistic, lambda = lambda, df = H1_Chi_edu_highschool$parameter)

H1_BF_edu_highschool  <- log(BF1)

H1_BF_edu_highschool
...
# sum(CFPS2018_gender$Proportion) ## 100
```
...
# sum(H1_gender_general2$Proportion) ## 100
...
H2_gender_bayes_general <- H2_gender_general %>% 
  ...
  dplyr::mutate(`Chinese subjects pool` = ifelse(Gender == "Male", 259134,298258)) %>% 
  dplyr::select(-2) %>% 
  dplyr::slice(-c(3:4)) %>% 
...
# calculate Chi-square
observed1 <- H2_gender_bayes_general$`Chinese subjects pool`
expected1 <- H2_gender_bayes_general$Census7
expected2 <- H2_gender_bayes_general$CFPS2018

## Census7
H2_Chi_gender_Census7_general <- chisq.test(x = observed1, p = expected1)
H2_Chi_gender_Census7_general

## CFPS2018
H2_Chi_gender_CFPS2018_general <- chisq.test(x = observed1, p = expected2)
H2_Chi_gender_CFPS2018_general


# calculate BF
## Census7
lambda <- UMPBT(df = H2_Chi_gender_Census7_general$parameter,thresh = 6)

BF1 <- froot_bf(x = H2_Chi_gender_Census7_general$statistic, lambda = lambda, df = H2_Chi_gender_Census7_general$parameter)

H2_BF_gender_Census7_general <- log(BF1)

H2_BF_gender_Census7_general


## CFPS2018
lambda <- UMPBT(df = H2_Chi_gender_CFPS2018_general$parameter,thresh = 6)

BF2 <- froot_bf(x = H2_Chi_gender_CFPS2018_general$statistic, lambda = lambda, df = H2_Chi_gender_CFPS2018_general$parameter)

H2_BF_gender_CFPS2018_general  <- log(BF2)

H2_BF_gender_CFPS2018_general 


## rm 
rm(observed1,expected1,expected2,BF1,BF2)
...
In this part, I calculate data based on sepcial terget population

```{r eval=FALSE, include=FALSE}
H2_gender_college <-  Journal_gender_special_college %>% 
  dplyr::summarise(Male_Num = sum(Male_Num,na.rm = TRUE),
                   Female_Num = sum(Female_Num,na.rm = TRUE),
                   Male_Proportion = round((Male_Num/(Male_Num + Female_Num))*100,4),
                   Female_Proportion = round((Female_Num/(Male_Num + Female_Num))*100,4),
                   Data_Source = "Target_population_special_College")

# sum(H2_gender_college$Male_Proportion) + sum(H2_gender_college$Female_Proportion) ## 100
...
H2_gender_bayes_special_college <- data.frame(Gender = c("Male","Female"),
                                              Journal = c(44.8077,55.1923),
                                              Outline_Women = c(50,50))

H2_BF_gender_special_college <- BayesMultiNomial(dataset = H2_gender_bayes_special_college,
                                                 factor = "Gender",
                                                 observed = "Journal",
...
# sum(H2_age_general$Proportion) ## 100.0001 fill to percent 100?
```
...
```{r}
##凡是BTS数据完善后涉及到合并Journal和BTS的都需要重新替换数据。切记切记！！！！！请注意本段并未比较CFPS
...
H2_age_Psystages_bayes_general <- bind_rows(BTS_age_PsyStages_CHN_general,Sim_Journal_age_PsyStages3_general_prop) %>% 
  ...
  tidyr::pivot_wider(names_from = Data_Source,
                     values_from = c(Proportion, n)) %>% 
  dplyr::select(1,4,3) %>% 
  dplyr::rename(`Chinese subjects pools` = 2,
                Census7 = 3) %>% 
  dplyr::mutate(Census7 = Census7 * 0.01)


# calculate Chi-square
observed1 <- H2_age_Psystages_bayes_general$`Chinese subjects pools`
expected1 <- H2_age_Psystages_bayes_general$Census7


## Census7
H2_Chi_age_Psystages_general  <- chisq.test(x = observed1, p = expected1)
H2_Chi_age_Psystages_general 


# calculate BF
## Census7
lambda <- UMPBT(df = H2_Chi_age_Psystages_general$parameter,thresh = 6)

BF1 <- froot_bf(x = H2_Chi_age_Psystages_general$statistic, lambda = lambda, df = H2_Chi_age_Psystages_general$parameter)

H2_BF_age_Psystages_general  <- log(BF1)

H2_BF_age_Psystages_general 
...
# calculate Chi-square
observed1 <- H2_age_inteval10_bayes_general$`Chinese subjects pools`
expected1 <- H2_age_inteval10_bayes_general$Census7


## Census7
H2_Chi_age_inteval10_general  <- chisq.test(x = observed1, p = expected1)
H2_Chi_age_inteval10_general 

# calculate BF
## Census7
lambda <- UMPBT(df = H2_Chi_age_inteval10_general$parameter,thresh = 6)

BF1 <- froot_bf(x = H2_Chi_age_inteval10_general$statistic, lambda = lambda, df = H2_Chi_age_inteval10_general$parameter)

H2_BF_age_inteval10_general  <- log(BF1)

H2_BF_age_inteval10_general 
...
Census7_college <- Census7_edu_adults %>% 
  dplyr::summarise(Below_college = sum(across(3:7)),
                   College_and_above = sum(across(8:11))) %>% 
...
# sum(Census7_college$Proportion) ## 100


Census7_high_school <- Census7_edu_adults %>% 
  dplyr::summarise(Below_high_school = sum(across(3:6)),
                   High_school_and_above = sum(across(7:11))) %>% 
  tidyr::pivot_longer(cols = c(1,2),
                      names_to = "Educational_attainment",
                      values_to = "Num") %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
...
# sum(Census7_high_school$Proportion) ## 100
```
...
CFPS2018_edu_college <- data.frame(Educational_attainment = c("Below_college","College_and_above"),
                                   Num = c(2846+756+3598+5273,3058+1648+1335+112+7)) %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "CFPS2018")

# sum(CFPS2018_edu_college$Proportion) ## 100


CFPS2018_edu_high_school <- data.frame(Educational_attainment = c("Below_high_school","High_school_and_above"),
                                   Num = c(2846+756+3598+5273+3058,1648+1335+112+7)) %>% 
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "CFPS2018")

# sum(CFPS2018_edu_high_school$Proportion) ## 100
```
...
Chinese_subjects_pools_edu_college <- H1_edu_college %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(Proportion = round((Num/sum(Num))*100,4),
                Data_source = "Chinese subjects pools")

# sum(Chinese_subjects_pools_edu_college$Proportion) ## 100
...
H2_edu_college_bayes <- H2_edu_college %>% 
  tidyr::pivot_wider(names_from = Data_source, values_from = c(Num,Proportion)) %>% 
...
# calculate Chi-square
observed1 <- H2_edu_college_bayes$`Chinese subjects pools`
expected1 <- H2_edu_college_bayes$Census7
expected2 <- H2_edu_college_bayes$CFPS2018

## Census7
H2_Chi_Census7_edu_college <- chisq.test(x = observed1, p = expected1)
H2_Chi_Census7_edu_college

## CFPS2018
H2_Chi_CFPS2018_edu_college  <- chisq.test(x = observed1, p = expected2)
H2_Chi_CFPS2018_edu_college 


# calculate BF
## Census7
lambda <- UMPBT(df = H2_Chi_Census7_edu_college$parameter,thresh = 6)

BF1 <- froot_bf(x = H2_Chi_Census7_edu_college$statistic, 
                lambda = lambda, 
                df = H2_Chi_Census7_edu_college$parameter)

H2_BF_Census7_edu_college  <- log(BF1)

H2_BF_Census7_edu_college 


## CFPS2018
lambda <- UMPBT(df = H2_Chi_CFPS2018_edu_college$parameter,thresh = 6)

BF2 <- froot_bf(x = H2_Chi_CFPS2018_edu_college$statistic, 
                lambda = lambda, 
                df = H2_Chi_CFPS2018_edu_college$parameter)


H2_BF_CFPS2018_edu_college <- log(BF2)

H2_BF_CFPS2018_edu_college 
...
Journal_region_single <- Journal %>% 
 ...
 #   dplyr::distinct(Article_IDs) ## single 519 papers + multiple 10
...
# sum(CFPS2018_region$Proportion) ## 100.0001 fill to percent 100?
```
...
H2_region <- H2_Fig_Census7_region + H2_Fig_CFPS2018_region + H2_Fig_region_general +
...
ggsave("H2_region .pdf", H2_region , device = "pdf", width=10, height = 15)
...
H2_region_bayes <- bind_rows(Journal_region_general_tab2,Census7_region,CFPS2018_region) %>% 
  dplyr::select(1,3,5) %>% 
  dplyr::mutate(Data_Source = ifelse(Data_Source == "Journal_region_total","Journal",Data_Source)) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion) %>% 
...
H2_BF_Journal_Census7_region_general <- BayesMultiNomial(dataset = H2_region_bayes,
                                          factor = "Region",
                                          observed = "Journal_region_general_population",
                                          expected = "Census7")

H2_BF_Journal_CFPS2018_region_general <- BayesMultiNomial(dataset = H2_region_bayes,
                                          factor = "Region",
                                          observed = "Journal_region_general_population",
                                          expected = "CFPS2018")
```
...
BTS_gender_allcountry <- lapply(H3_BTS_gender_list, function(df){
...
## Add Chinese data
Journal_BTS_PsyStages_CHN <- Sim_Journal_age_PsyStages3_general_prop %>% 
  dplyr::rename(n = 2) %>% 
  # dplyr::bind_rows(BTS_age_PsyStages_CHN_total) %>%  
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(n=sum(n)) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,4),
                ISO3 = "CHN")
...
## Add Chinese data
Journal_BTS_interval10_CHN <- Sim_Journal_age_interval103_general_prop %>% 
  dplyr::rename(n = 2) %>% 
  # bind_rows(BTS_age_interval10_CHN_total) %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(n=sum(n)) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,4),
                ISO3 = "CHN")
...
H3_BF_PsyStages <- H3_BF_PsyStages %>%  
  dplyr::mutate(Countries = factor(Countries, levels = H3_BF_PsyStages_factor)) %>%  ## 最好创建age自身的factor
  tidyr::pivot_wider(names_from = "Countries",values_from = "Proportion") %>%
...
H3_Fig_gender_age_suppl <- H3_Fig_BF_interval10 + H3_Fig_interval10 +
   plot_annotation(tag_levels =list(c("S3a","S3b")))  & 
  theme(plot.tag = element_text(size = 18, face = "bold"))
...
dplyr::filter(Article_IDs,Article_Title,Study_Number_N,Subjects_Group_N,
Target_Population_N,Sample_Type_N,Subjects_Recruitment_Method_N)## Reasons for results
```{r}
Journal_subjects_recruitment <- Journal %>% 
