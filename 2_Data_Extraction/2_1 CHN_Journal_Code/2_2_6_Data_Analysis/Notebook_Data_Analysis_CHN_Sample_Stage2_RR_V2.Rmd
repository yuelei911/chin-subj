...
Chinese_sum <- BTS_Overview2 %>% 
  dplyr::filter(ISO2 %in% c("TW","HK","MO","CN")) %>% 
  dplyr::summarise(n=sum(n)) ## 29006
29006/1367084
...
Journal_gender_Infants_toddlers <- Journal_gender_Infants_toddlers_TP_IDs %>% 
  dplyr::summarise(Male_Num = sum(as.numeric(Male_Num)),
                   ...
                   Male_Proportion = Male_Num/(Male_Num+Female_Num)*100,
                   Female_Proportion = Female_Num/(Male_Num+Female_Num)*100) ## Male 51.57, Female 48.43
...
BTS_gender_CHN <- lapply(BTS_gender_list, function(df){
  ...
  df %>% 
    dplyr::filter(grepl("CHN|TWN|MAC|HKG", ISO3, ignore.case = TRUE)) %>% 
...
H1_gender_general <- BTS_gender_CHN_general %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(57570,48059),
                 Proportion = c(54.50,45.50),Data_Source = "Journal")   ## Here, enter data manually, data from data.frame "Journal_gender_general".
...
Lucca_gender_special <- BTS[[19]] %>% 
  ...
  dplyr::mutate(Porportion = n/sum(n)*100, ## Male 56.99, Female 43.01
                Data_Source = "Lucca")
