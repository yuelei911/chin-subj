...
Census6_Age$ageBins[20] <- ">=95"

Census6_Age <- Census6_Age %>% 
  dplyr::group_by(ageBins) %>% 
...
Census7_Age$ageBins[20] <- ">=95" 
Census7_Age$Proportion[20] <- 0.06+0.01


Census7_Age <- Census7_Age %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(Proportion = sum(Proportion)) %>% 
  dplyr::mutate(Data_Source = "Census7") %>% 
  dplyr::ungroup() %>% 
...
```{r}
H2_Fig_age <- ggplot(data= Census7_Age, 
