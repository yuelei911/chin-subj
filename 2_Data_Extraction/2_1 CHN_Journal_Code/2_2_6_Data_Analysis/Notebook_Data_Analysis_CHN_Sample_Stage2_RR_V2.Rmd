...
And then, I load the data. The data includes data from the first Stage 1 of the pre-registration report (primarily Census data and CFPS2018 data), Chinese journal coding data, and data from Big Team Science (BTS).
# Load data
...
rm(df,df_PSA001,df_stage2_Journal_Code,Group1_bind,Group24567_bind,Group3_bind,df_regionCode,df_census6)
```


## Load the data of BTS
Here, we first convert some data that is not in .xlsx format.
```{r}
library(haven)
...
Next, I load all BTS data and process BTS data.
...
Uniform country codes facilitate subsequent data processing and analysis, especially for batch processing and analysis of data. Thus, I create Create a unified table of country codes.
```{r}
...
The third hypothesis requires countries to be divided into WEIRD and non-WEIRD, so I create a data.frame of WEIRD  countries to facilitate subsequent analysis and visualization
```{r}
WEIRD_code <- codelist %>% 
...
In this part, I try to batch country code.
```{r}
...
Here, I use self-defined function in Stage1 to calculate Bayesian multinomial test
...
I begin by analyzing the basic case of the two data sets (BTS and Chinese journals).
...
In this section, I begin by testing and analyzing three main hypotheses.
# Test three hypotheses
## Test the 1st hypothesis
### Gender
Process gender data from Chinese journal.
```{r warning=FALSE}
## Minor revision
...
At the end of gender data processing for hypothesis 1, I remove some data sets to reduce data set clutter.
```{r}
...
### Age
Reprocess age data from Chinese journal.
```{r warning=FALSE}
...
Since raw data is not available for papers in Chinese journals, we simulated the raw data using the method used in the Stges 1 of registration report. It is then combined with BTS and compared with census data.

First, I created a function to group ages into five-year intervals for the purpose of creating a Population pyramid chart.
```{r}
...
Meanwhile, I create func to process age data simulation in 10-year-old segment.
```{r}
...
Process age for Bayesian multinomial test. First of all, I process BTS CHN age in psychological development stages.
```{r}
...
Secondly, I process BTS CHN age in 10-year-old segment.
```{r}
...
And then, I filter BTS CHN data.
```{r}
...
Next, I Combine two CHN data to Bayesian multinomial test in two different classfications (PsyStages and 10-year-old). One is PsyStages.
```{r}
...
#### Visualize age data from Chinese Subjects pool, census7 and CFPS2018
```{r}
...
#### Combine Chinese age data of BTS and Chinese papers.
```{r}
...
#### Processing of provincial distribution of population for CFPS2018
```{r}
...
#### Bayesian multinomial test for region from journal, census 7 and CFPS2018
```{r}
...
#### Visualize age data for countries around the world in BTS
```{r}
...
#### Visualization age data of all countries
```{r}
...
# ggsave("Fig_Journal_BTS_ALL_interval10.pdf", Fig_Journal_BTS_ALL_interval10, device = "pdf", width=18, height = 9)

H3_Fig_interval10
```


### define a function for calculating BF for multiple countries
```{r}
...
# ggsave("Fig_BF_gender.pdf", Fig_BF_gender, device = "pdf", width=18, height = 18)
...
#### Visualization BF age (PsyStages)
```{r warning=FALSE}
...
# ggsave("Fig_BF_PsyStages.pdf", Fig_BF_PsyStages, device = "pdf", width=18, height = 18)
...
#### Visualization BF age (interval10)
```{r warning=FALSE}
