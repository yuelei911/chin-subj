...
Region_map_colour <- readxl::read_xlsx("colour.xlsx") %>% 
  dplyr::mutate(QUHUADAIMA = as.character(QUHUADAIMA),
                Journal = 0,
                Journal_target_general = 0,
                Journal_target_special = 0,
                Census7 = 0,
                CFPS2018 = 0)
...
```{r}
colour <- colour %>% 
  dplyr::mutate(Journal_target_population23 = 0)


colour$Journal_target_population23[colour$province %in% Target_population23_region_count $Subjects_Recruitment_Area[df_CHN_Journal_Province_count$Count_Bins == "<1"]] <- 1

colour$Journal_target_population23[colour$province %in% Target_population23_region_count $Subjects_Recruitment_Area[df_CHN_Journal_Province_count$Count_Bins == "1~5"]] <- 2

colour$Journal_target_population23[colour$province %in% Target_population23_region_count $Subjects_Recruitment_Area[df_CHN_Journal_Province_count$Count_Bins == "5~10"]] <- 3

colour$Journal_target_population23[colour$province %in% Target_population23_region_count $Subjects_Recruitment_Area[df_CHN_Journal_Province_count$Count_Bins == ">=10"]] <- 4



colour <- colour %>%
  dplyr::mutate(Journal_target_population23 = factor(Journal_target_population23, 
                                          levels = c("0","1", "2", "3", "4")))
...
Journal_Area <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Journal_target_population23))) +
  scale_fill_manual(values = c("#FFFFFF", "#C6DBEF","#6BAED6", "#2171B5", "#08306B"),
                    breaks = c("0","1","2", "3", "4"),
                    labels = c("0", "<1%","<5%", "<10%",">=10%"),drop = FALSE) +
  geom_sf(data = gjx) +
