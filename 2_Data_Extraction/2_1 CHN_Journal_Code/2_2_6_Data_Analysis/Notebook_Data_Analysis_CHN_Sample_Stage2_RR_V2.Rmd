...
Journal_region_tab2 <- Journal_region_tab %>% 
  dplyr::mutate(Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "Journal_region_new")%>% 
  dplyr::rename(Region = 1)
```
...
Region_map_data <-  bind_rows(Census7_region,CFPS2018_region,Journal_region_total,
          Journal_region_general,Journal_region_special,Journal_region_tab2)
...
Journal_edu2_tab <- Journal_edu2 %>% 
  dplyr::summarise(tab10 = sum(is.na(Collage_and_above)),
                   tab11 = sum(is.na(Below_collage)),
...
## the first class, collage
Journal_collage <- Journal_edu2 %>% 
  dplyr::select(-c(5:9,12:15)) %>% 
  dplyr::filter(!is.na(Collage_and_above) & !is.na(Below_collage))

##  both > 1
Journal_collage_integer <- Journal_collage  %>%
  dplyr::filter(Collage_and_above > 1 & Below_collage > 1) %>%
  dplyr::summarise(Collage_and_above = sum(Collage_and_above),
                   Below_collage = sum(Below_collage))
  

## both < 1
Journal_collage_decimal<- Journal_collage  %>%
  dplyr::filter(Collage_and_above <= 1 & Below_collage <= 1) %>% 
  dplyr::mutate(Total_Collage_and_above = Collage_and_above + Below_collage) %>% 
  dplyr::mutate(Collage_and_above_tab = floor(Sample_Size * Collage_and_above),
                Below_collage_tab = ifelse(Total_Collage_and_above == 1, Sample_Size - Collage_and_above_tab, floor(Sample_Size * Below_collage))) %>% 
  dplyr::mutate(tab = Collage_and_above_tab + Below_collage_tab) %>% 
  dplyr::summarise(Collage_and_above = sum(Collage_and_above),
                   Below_collage = sum(Below_collage))

## combine both class
Journal_collage2 <-  bind_rows(Journal_collage_decimal, Journal_collage_integer) %>% 
  dplyr::summarise(Collage_and_above = sum(Collage_and_above),
                   Below_collage = sum(Below_collage)) %>% 
  tidyr::pivot_longer(cols = c(Collage_and_above,Below_collage),
...
Journal_high_school_decimal <- Journal_high_school  %>%
  ...
  dplyr::mutate(High_school_and_above_tab = floor(Sample_Size * High_school_and_above),
                Below_high_school_tab = ifelse(Total_High_school == 1, Sample_Size - High_school_and_above, floor(Sample_Size * Below_high_school))) %>% 
...
## combine both class
Journal_high_school2 <-  bind_rows(Journal_high_school_integer, Journal_high_school_decimal) %>% 
  dplyr::summarise(High_school_and_above = sum(High_school_and_above),
                   Below_high_school = sum(Below_high_school)) %>% 
  ...
  dplyr::mutate(M_edu_year = as.numeric(M_edu_year),
                Edu_year_cate = ifelse(M_edu_year <= 6, "Elementary School",
                                       ifelse(M_edu_year <= 9, "Middle School",
                                              ifelse(M_edu_year <= 12, "High School","Collage")))) %>% 
  dplyr::arrange(M_edu_year)

Journal_edu2_year2 <-  Journal_edu2_year %>% 
  dplyr::slice(-5) %>% 
...
# sum(Journal_edu2_year2$Sample_Size) #2787
...
```{r}
## collage
H2_Fig_collage <- ggplot(data = Journal_collage2,aes(Educational_Attainment,Proportion,fill = Educational_Attainment,Proportion))+
  geom_col()+
  theme_classic()+
...
H2_Fig_edu_year2 <- ggplot(data = Journal_edu2_year2,aes(Edu_year_cate,Proportion,fill = Edu_year_cate))+
  geom_col()+
  xlab("Educational attainment (2,787 subjects)")+
  ylab("")+
  theme_classic()+
...
H2_Fig_edu2 <- H2_Fig_collage + H2_Fig_high_school + H2_Fig_edu_year2 +
   plot_annotation(tag_levels = "A")  & 
  theme(plot.tag = element_text(size = 18, face = "bold"))


ggsave("H2_Fig_edu2.pdf", H2_Fig_edu2, device = "pdf", width=18, height = 9)
