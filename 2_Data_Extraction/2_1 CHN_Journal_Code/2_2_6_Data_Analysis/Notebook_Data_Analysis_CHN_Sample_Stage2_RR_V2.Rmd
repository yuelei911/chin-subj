...
## total population
BTS_gender_total_CHN <- BTS_gender_CHN %>%
...
H1_gender_total <- H1_gender_total %>% 
  dplyr::mutate(Data_Source = factor(Data_Source,
                                     levels = H1_gender_total_factor))


## General population
BTS_gender_general_CHN <- BTS_gender_CHN %>%
...
H1_gender_general <- BTS_gender_general_CHN %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(273804,234047),
                 Proportion = c(54,46),Data_Source = "Journal")   ## Here, enter data manually.

H1_gender_general_factor <- H1_gender_general %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(Data_Source)
...
H1_gender_general <- H1_gender_general %>% 
  dplyr::mutate(Data_Source = factor(Data_Source,
                                     levels = H1_gender_general_factor))
```
...
```{r}
H1_gender_total_bayes <- H1_gender_total %>% 
  dplyr::select(1,3:4) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion)

H1_BF_gender_total <- BayesMultiNomial(dataset = H1_gender_total_bayes,
                                             factor = "Gender",
...
```{r}
H1_gender_general_bayes <- H1_gender_general %>% 
  dplyr::select(1,3:4) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion)
...
```{r}
rm(H1_gender_total_bayes,H1_gender_total_factor,H1_gender_general_factor,i,BTS_name)
```
...
H1_age_twosource_difference <- H1_age %>% 
