...
merged_list <- lapply(BTS_data22, function(df) {  
     
    if (any(grepl("CN", df$Country, ignore.case = TRUE))) {  
       
      merge(df, countrycodelist_iso2_iso3, by.x = "Country", by.y = "ISO2", all.x = TRUE)  
    } else if (any(grepl("CHN", df$Country, ignore.case = TRUE))) {  
      
      merge(df, countrycodelist_iso2_iso3, by.x = "Country", by.y = "ISO3", all.x = TRUE)  
    } else {  
      
          merge(df, countrycodelist_iso2_iso3, by.x = "Country", by.y = "Countryfullname", all.x = TRUE) 
    }  
  return(df)
...
```{r}
Journal_report <- ggplot(df_CHN_report,aes(col_names,proportion,fill=Var1))+
  geom_col()+
  labs(x="",y="%")+
  theme_classic()+
...
ggsave("Journal_report.pdf", Journal_report, device = "pdf", width=16, height = 9)
```
...
BTS_CN_gender <- lapply(BTS_gender_list, function(df){
  
  df %>% 
    dplyr::filter(grepl("CN|CHN|CHINA", Country, ignore.case = TRUE))

})

BTS_CN_gender <- do.call(rbind, BTS_CN_gender)  
...
Journal_BTS_CN_gender <- BTS_CN_gender %>% 
  dplyr::mutate(Proportion = Proportion * 100) %>% 
  bind_rows(df_census7_Gender,df_CFPS2018_Gender,df_CHN_Journal_Gender_Total2) %>% 
  dplyr::mutate(Gender =  stringr::str_to_title(Gender))

Journal_BTS_CN_gender_factor <- Journal_BTS_CN_gender %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(Data_Source)

Journal_BTS_CN_gender <- Journal_BTS_CN_gender %>% 
  dplyr::mutate(Data_Source = factor(Data_Source,
                                     levels = Journal_BTS_CN_gender_factor))
```


#### Visualize gender data for Chinese journals and BTS
```{r}
Journal_BTS_CN_gender <- ggplot(data = Journal_BTS_CN_gender,aes(Data_Source,Proportion,fill = Gender))+
  geom_col()+
...
ggsave("Journal_BTS_CN_gender.pdf", Journal_BTS_CN_gender, device = "pdf", width=18, height = 9)

Journal_BTS_CN_gender
```
...
#ggsave("Journal_age.pdf", Journal_age, device = "pdf", width=18, height = 9)

Journal_age
```


#### Create a age function for batch processing
```{r}
...
```{r}
## Arechar_et_al_2023
df_Arechar_et_al_2023 <- BTS_data[[1]]
...
#### Visualize age data from countries around the world in BTS
```{r}
### Arechar_et_al_2023
Arechar_et_al_2023_CN_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
...
Arechar_et_al_2023_CN_age


### Azevedo_et_al_2022_mainland
Azevedo_et_al_2022_mainland_CN_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
...
Azevedo_et_al_2022_mainland_CN_age


## Azevedo_et_al_2022_taiwan
Azevedo_et_al_2022_taiwan_CN_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
...
Azevedo_et_al_2022_taiwan_CN_age


## Combined graph
Journal_BTS_CN_age <-  Journal_age + Arechar_et_al_2023_CN_age + 
  Azevedo_et_al_2022_mainland_CN_age + Azevedo_et_al_2022_taiwan_CN_age +
  plot_layout(ncol = 2, nrow = 2,heights = c(4,4))+
  patchwork::plot_annotation(tag_levels = 'A')

ggsave("Journal_BTS_CN_age.pdf", Journal_BTS_CN_age, device = "pdf", width=18, height = 9)
...
### Gender
####  Create a gender function for batch processing
```{r}
...
#### Process age data all countries in BTS
```{r}
## df_Arechar_et_al_2023
