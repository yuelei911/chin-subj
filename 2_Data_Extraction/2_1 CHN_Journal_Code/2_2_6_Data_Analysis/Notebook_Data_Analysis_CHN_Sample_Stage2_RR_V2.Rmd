...
### Load BTS data
```{r message=FALSE, warning=FALSE}
file_list <- list.files(here("D://chin-subj//2_Data_Extraction//2_1 CHN_Journal_Code//2_2_6_Data_Analysis"),
                        pattern = ".csv",
                        full.names = TRUE,
                        recursive = TRUE)
...
for (file in file_list) {  
  ...
  BTS_data[[tools::file_path_sans_ext(basename(file))]] <- BTS_file  

}  

BTS_data <- BTS_data[grepl("BTS_",names(BTS_data),ignore.case = TRUE)]
rm(BTS_file)
```


### Create a function to batch process gender
```{r}
...
##create a function to calculate gender proportion
```{r}
Func_gender_proportion <- function(data){
