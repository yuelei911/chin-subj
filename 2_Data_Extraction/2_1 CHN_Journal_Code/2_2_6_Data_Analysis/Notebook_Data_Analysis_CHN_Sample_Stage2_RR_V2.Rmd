...
I found some little error, I recode them before informal process data.
```{little error}
...
## 20215075
df_stage2_Journal_Code2$Sample_Size[1420] <- 310
df_stage2_Journal_Code2$Male_Num[1420] <- 310
df_stage2_Journal_Code2$Female_Num[1420] <- 0
df_stage2_Journal_Code2$M_age[1420] <- 36.11
df_stage2_Journal_Code2$SD_age[1420] <- 4.91
df_stage2_Journal_Code2$Sample_Type_N[1420] <- 4


## I found some outliers for M_age, so I processed this part of the data first.
## 20185036
df_stage2_Journal_Code2$Sample_Type_N[1083] <- 2
## 20183336
df_stage2_Journal_Code2$Sample_Type_info_N[184] <-
  "采用整群抽样的方法从山东省济南市某高校选取大学生为被试"

## 20215065
df_stage2_Journal_Code2$Sample_Type_N[1191:1192] <- 2
## 20184083
df_stage2_Journal_Code2$M_age[408] <- "男性69.14;女性67.16"

## 20084023
df_stage2_Journal_Code2$Sample_Type_N[301] <- 5

df_stage2_Journal_Code2$Gender[352:353] <- 0 ## 20182022 Original article gender report error, 0 here

## little error 20181223
df_stage2_Journal_Code2$M_age[1501] <- 31.6
...
## 20181022
df_stage2_Journal_Code2$Ethnicity_info[769] <- "汉族"

### Article_IDs 20181113 
df_stage2_Journal_Code2$Ethnicity[1292:1293] <- 1
df_stage2_Journal_Code2$Ethnicity_info[1292:1293] <- "维吾尔族"

### Article_IDs 20211018
df_stage2_Journal_Code2$Ethnicity[1635:1636] <- 1
df_stage2_Journal_Code2$Ethnicity_info[1635:1636] <- "藏族"

### Article_IDs 20213168
df_stage2_Journal_Code2$Ethnicity[1604] <- 1
df_stage2_Journal_Code2$Ethnicity_info[1604] <- "汉族;少数民族"

### Article_IDs 20181017
df_stage2_Journal_Code2$Ethnicity[761:762] <- 1
df_stage2_Journal_Code2$Ethnicity_info[761:762] <- "蒙古族"

### Article_IDs 20211013
df_stage2_Journal_Code2$Ethnicity[1588:1589] <- 1
df_stage2_Journal_Code2$Ethnicity_info[1588:1589] <- "锡伯族"
```
...
Journal_key_variable <- df_stage2_Journal_Code2 %>% 
...
df_stage2_Journal_Code_Gender_integer_add <- data.frame(
  Article_IDs = c("20185082","20213045","20213048","20213048"),
...
```{r}
Journal_region <- df_stage2_Journal_Code2 %>% 
  dplyr::filter(Subjects_Recruitment_Area != 0) %>% 
  dplyr::select(Article_IDs,Study_Number_N,Subjects_Group_N,
                Target_Population_N,Sample_Type_N,Subjects_Recruitment_Area) %>% 
  dplyr::mutate(Subjects_Recruitment_Area = gsub("[；、,:]", ";",Subjects_Recruitment_Area)) %>% 
  tidyr::separate_rows(Subjects_Recruitment_Area,sep = ";") %>% 
  dplyr::mutate(Subjects_Recruitment_Area =
                  gsub("自治区|回族自治区|维吾尔自治区|壮族自治区","",Subjects_Recruitment_Area)) %>% 
...
Journal_target_general_region <- Journal_region %>% 
  ...
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
                Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "Journal_general_region") %>% 
  dplyr::rename(Region = 1)

Journal_target_general_region


## Target population is special
Journal_target_special_region <- Journal_region %>% 
  dplyr::filter(Target_Population_N == 1) %>% 
  dplyr::count(Subjects_Recruitment_Area) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
                Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "Journal_special_region") %>% 
  dplyr::rename(Region = 1)

Journal_target_special_region
...
```{r}
## combine region data
Region_map_data <-  bind_rows(Census7_region,CFPS2018_region,Journal_total_region,
          Journal_target_general_region,Journal_target_special_region)
...
Journal_Total_Area <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_total_region))) +
...
## Chinese journal papers
Journal_general_Area <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_general_region))) +
  scale_fill_manual(values = c("#FFFFFF", "#C6DBEF","#6BAED6", "#2171B5", "#08306B"),
                    breaks = c("0","1","2", "3", "4"),
                    labels = c("0", "<1%","<5%", "<10%",">=10%"),drop = FALSE) +
  geom_sf(data = gjx) +
  labs(title = "Journal general population area distribution") +
  theme(plot.title = element_text(color = "black",size = 16,
                                  face = "bold",vjust = 0.1, hjust = 0.5),
        legend.title = element_blank(),
        legend.position = c(0.2, 0.2),
...
## Chinese journal papers
Journal_special_Area <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_special_region))) +
  scale_fill_manual(values = c("#FFFFFF", "#C6DBEF","#6BAED6", "#2171B5", "#08306B"),
                    breaks = c("0","1","2", "3", "4"),
                    labels = c("0", "<1%","<5%", "<10%",">=10%"),drop = FALSE) +
  geom_sf(data = gjx) +
  labs(title = "Journal sepcial population area distribution") +
  theme(plot.title = element_text(color = "black",size = 16,
                                  face = "bold",vjust = 0.1, hjust = 0.5),
        legend.title = element_blank(),
        legend.position = c(0.2, 0.2),
...
H2_Area <- (Census7_Area + CFPS2018_Area)/( 
  Journal_Total_Area + Journal_special_Area + Journal_general_Area) +
...
After analyzing data at a general level, I analyzed the data based on the target population.

## Gender

Try to analyze data that the target population is the general population.
```{r message=FALSE, warning=FALSE}
Ta <- df_stage2_Journal_Code2 %>% 
  dplyr::select(Article_IDs,Study_Number,Subjects_Group,Age,M_age)
  



Target_population23 <- df_stage2_Journal_Code2 %>% 
  dplyr::filter(Target_Population_N != 1) %>%
  dplyr::mutate(Sample_Size = as.numeric(Sample_Size),
                Gender = as.numeric(Gender))


## Filter gender number < 1. 
Target_population23_gender_decimal <- Target_population23 %>% 
  dplyr::select(Article_IDs,Study_Number,Subjects_Group_N,Sample_Size,Gender,
                Male_Num,Female_Num,Remark1,Remark2,Remark3) %>% 
...
## Filter gender number > 1.  
Target_population23_gender_integer <- Target_population23 %>% 
  dplyr::select(Article_IDs,Study_Number,Subjects_Group_N,Sample_Size,Gender,
                Male_Num,Female_Num,Remark1,Remark2,Remark3) %>% 
  dplyr::filter(Gender == 1) %>% 
  dplyr::filter(Male_Num > 1 | Female_Num > 1) %>% 
...
## Check whether data can be matched.
Target_population23_gender_twotype <-
  bind_rows(Target_population23_gender_decimal,Target_population23_gender_integer)

Target_population23_gender_remove <- Target_population23 %>% 
  dplyr::select(Article_IDs,Study_Number,Subjects_Group_N,Sample_Size,Gender,
                Male_Num,Female_Num,Remark1,Remark2,Remark3) %>% 
  filter(Gender == 1) %>% 
   dplyr::mutate(Male_Num = as.numeric(Male_Num),
...
Target_population23_gender_anti <- anti_join(Target_population23_gender_remove,Target_population23_gender_twotype) 
  

## In this step, I found that only 20213048 were forced to convert to NA during the conversion of data format.
Target_population23_gender_add <- data.frame(
  Article_IDs = c("20213048","20213048"),
  Study_Number = c(1,2),
  Subjects_Group_N = c(NA,NA),
  Sample_Size = c(68,59),
  Gender = 1,
  Male_Num = c(31,28),
  Female_Num = c(37,31)) %>% 
  dplyr::mutate(Study_Number = as.character(Study_Number))


## Process gender number is decimal
Target_population23_gender_decimal2 <- Target_population23_gender_decimal %>%
  dplyr::rename(Male_Proportion = Male_Num,
                Female_Proportion =  Female_Num) %>% 
  dplyr::mutate(Male_Num = floor(Sample_Size*Male_Proportion),
                Female_Num = Sample_Size - Male_Num) %>% 
  dplyr::select(1:5,8:9)
  
# names(Target_population23_gender_decimal2)

## Combine two type data.
Target_population23_gender <- dplyr::bind_rows(Target_population23_gender_decimal2,
                                               Target_population23_gender_integer,
                                               Target_population23_gender_add)

Target_population23_gender <- Target_population23_gender %>% 
  dplyr::summarise(Male_Num = sum(Male_Num,na.rm = TRUE),
                   Female_Num = sum(Female_Num,na.rm = TRUE),
                   Male_Proportion = round(Male_Num/(Male_Num + Female_Num),2)*100,
                   Female_Proportion = round(Female_Num/(Male_Num + Female_Num),2)*100,
                   Data_Source = "Chinese papers")   ## male_p 45 female_p 55
```


Here, I remove some data sets to reduce data set clutter.
```{r}
rm(Target_population23_gender_twotype,
   Target_population23_gender_remove,
   Target_population23_gender_anti,
   Target_population23_gender_decimal,
   Target_population23_gender_decimal2,
   Target_population23_gender_integer,
   Target_population23_gender_add)
```


try to analyze the data that the target population is specific population. specifically, the target population is infant and young children.  
```{r message=FALSE, warning=FALSE}
Target_population1_sampletype3 <- df_stage2_Journal_Code2 %>% 
  dplyr::filter(Target_Population_N == 1 & Sample_Type_N == 3) %>%
  dplyr::select(Article_IDs,Study_Number,Subjects_Group_N,Sample_Size,Gender,
                Male_Num,Female_Num,Remark1,Remark2,Remark3) %>% 
  dplyr::mutate(Sample_Size = as.numeric(Sample_Size),
                Gender = as.numeric(Gender))
...
## Filter gender number < 1. 
Target_population1_sampletype3_gender_decimal <- Target_population1_sampletype3  %>% 
  dplyr::mutate(Male_Num = as.numeric(Male_Num),
                Female_Num = as.numeric(Female_Num)) %>% 
  dplyr::filter(Gender == 1) %>% 
  dplyr::filter(Male_Num < 1 & Female_Num < 1) %>% 
...
## Filter gender number > 1.  
Target_population1_sampletype3_gender_integer <- Target_population1_sampletype3  %>% 
  dplyr::filter(Gender == 1) %>% 
  dplyr::filter(Male_Num > 1 | Female_Num > 1) %>% 
  dplyr::mutate(Male_Num = as.numeric(Male_Num),
                Female_Num = as.numeric(Female_Num))%>% 
...
## Check whether data can be matched.
Target_population1_sampletype3_gender_twotype <-
  bind_rows(Target_population1_sampletype3_gender_decimal,
            Target_population1_sampletype3_gender_integer)

Target_population1_sampletype3_gender_remove <- Target_population1_sampletype3 %>% 
  filter(Gender == 1) %>% 
   dplyr::mutate(Male_Num = as.numeric(Male_Num),
                Female_Num = as.numeric(Female_Num))

Target_population1_sampletype3_gender_anti <- anti_join(Target_population1_sampletype3_gender_remove,Target_population1_sampletype3_gender_twotype) 
  

## Multiple groups of data are removed during processing, so re-create the data.frame and then merge them.


Target_population1_sampletype3_gender_add <- data.frame(
  Article_IDs = c("20185082","20213045","20215075"),
  Study_Number = c(1,1,1),
  Subjects_Group_N = c(NA,NA,"父亲"),
  Sample_Size = c(33,81,310),
  Gender = 1,
  Male_Num = c(26,49,310),
  Female_Num = c(7,32,0)) %>% 
  dplyr::mutate(Study_Number = as.character(Study_Number))
...
Target_population1_sampletype3_gender_decimal2 <- Target_population1_sampletype3_gender_decimal %>%
  dplyr::rename(Male_Proportion = Male_Num,
                Female_Proportion =  Female_Num) %>% 
  dplyr::mutate(Male_Num = floor(Sample_Size*Male_Proportion),
                Female_Num = Sample_Size - Male_Num) %>% 
...
Target_population1_sampletype3_gender <- dplyr::bind_rows(
  Target_population1_sampletype3_gender_decimal2,
  Target_population1_sampletype3_gender_integer,
  Target_population1_sampletype3_gender_add)


Target_population1_sampletype3_gender <- Target_population1_sampletype3_gender %>% 
  dplyr::summarise(Male_Num = sum(Male_Num,na.rm = TRUE),
                   Female_Num = sum(Female_Num,na.rm = TRUE),
                   Male_Proportion = round(Male_Num/(Male_Num + Female_Num),2)*100,
                   Female_Proportion = round(Female_Num/(Male_Num + Female_Num),2)*100,
                   Data_Source = "Chinese papers")   ## male_p  54 female_p 46

# ## 可以分析的目前考虑的有性别、地域和学历。后两者其实也很难。
# 
# 
# general M 45 F 45
# T23 M 45 F 54
...
Here, I remove some data sets to reduce data set clutter.
```{r}
rm(Target_population1_sampletype3_gender_remove,
   Target_population1_sampletype3_gender_twotype,
   Target_population1_sampletype3_gender_anti,
   Target_population1_sampletype3_gender_decimal,
   Target_population1_sampletype3_gender_decimal2,
   Target_population1_sampletype3_gender_integer,
   Target_population1_sampletype3_gender_add)
```
...
try to analyze the age data of the general population.
```{r}
Target_population23_articleIDs <- Target_population23 %>% 
  dplyr::select(Article_IDs,Study_Number,Subjects_Group,Sample_Size,Age,M_age,SD_age) %>% 
  dplyr::filter(Age == 1 & !is.na(M_age) & ! is.na(SD_age) & !is.na(Sample_Size)) %>% 
  dplyr::distinct(Article_IDs)


# Target_population23_age_extract <- df_stage2_Journal_Code2 %>% 
#   dplyr::filter(Article_IDs %in% Target_population23_articleIDs$Article_IDs) %>% 
#   dplyr::select(Article_IDs,Study_Number,Subjects_Group,Target_Population_N)

Target_population23_age <- df_stage2_Journal_Code_Age2 %>% 
  dplyr::filter(Article_IDs %in% Target_population23_articleIDs$Article_IDs) %>% 
  dplyr::filter(Age == 1 & !is.na(M_age) & ! is.na(SD_age) & !is.na(Sample_Size)) %>% 
  dplyr::mutate(Sample_Size = as.numeric(Sample_Size))

# str(Target_population23_age)


## Use simulation function to process data.
Sim_Journal_target23_age_five1 <- lapply(1:nrow(Target_population23_age),function(i){
  
  Sim_Sens_mult_five(Mean0 = Target_population23_age$M_age[i],
                diff_age = 0,
                SD0 = Target_population23_age$SD_age[i],
                sample_N = Target_population23_age$Sample_Size[i])
  
  })


Sim_Journal_target23_age_five2 <- do.call(rbind,Sim_Journal_target23_age_five1)


Sim_Journal_target23_age_five3 <- Sim_Journal_target23_age_five2 %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(sim1 = round(sum(sim1,na.rm = TRUE))) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(ageBins =factor(ageBins,
...
## Calculate the sample difference between the coded data and the simulated data
sum(Target_population23_age$Sample_Size)-sum(Sim_Journal_target23_age_five3 $sim1,na.rm = TRUE)


## calculate prop
Sim_Journal_target23_age_five3_prop <- Sim_Journal_target23_age_five3 %>% 
  dplyr::mutate(Proportion = round((sim1/sum(sim1))*100,2),
                Data_Source = "Chinese papers")

## visualization
H2_Fig_target23_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = Sim_Journal_target23_age_five3_prop, aes(x=ageBins, 
                y = Proportion,group = 1,color = "Chinese papers"),size =1)+
  scale_fill_manual(values=c("Census 7"="#00BFC4")) +  
  scale_color_manual(values=c("Chinese papers" = "red")) +
...
ggsave("H2_Fig_target23_age.pdf", H2_Fig_target23_age, device = "pdf", width=18, height = 9)
