...
Journal_region_general <- Journal_region %>% 
  ...
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
                Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "Journal_general_population_region") %>% 
  dplyr::rename(Region = 1)

Journal_region_general

## Target population is special
Journal_region_special <- Journal_region %>% 
  dplyr::filter(Target_Population_N == 1) %>% 
  dplyr::count(Subjects_Recruitment_Area) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
                Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "Journal_special_population_region") %>% 
  dplyr::rename(Region = 1)

Journal_region_special
```
...
```{r}
## combine region data
Region_map_data <-  bind_rows(Census7_region,CFPS2018_region,Journal_region_total,
          Journal_region_general,Journal_region_special)
...
H2_Fig_region_total <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_total_population_region))) +
...
H2_Fig_region_general <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_general_population_region))) +
  scale_fill_manual(values = c("#FFFFFF", "#C6DBEF","#6BAED6", "#2171B5", "#08306B"),
                    breaks = c("0","1","2", "3", "4"),
                    labels = c("0", "<1%","<5%", "<10%",">=10%"),drop = FALSE) +
  geom_sf(data = gjx) +
...
## Chinese journal papers
H2_Fig_region_special <- ggplot() +
  geom_sf(data = China,aes(fill = factor(Color_Journal_special_population_region))) +
  scale_fill_manual(values = c("#FFFFFF", "#C6DBEF","#6BAED6", "#2171B5", "#08306B"),
                    breaks = c("0","1","2", "3", "4"),
                    labels = c("0", "<1%","<5%", "<10%",">=10%"),drop = FALSE) +
  geom_sf(data = gjx) +
...
H2_region <- (H2_Fig_Census7_region + H2_Fig_CFPS2018_region)/( 
  H2_Fig_region_total + H2_Fig_region_general + H2_Fig_region_special) +
...
Journal_edu2_tab <- Journal_edu2 %>% 
  dplyr::summarise(tab10 = sum(is.na(Higher_edu)),
                   tab11 = sum(is.na(Below_higher_edu)),
                   tab12 = sum(is.na(Compulsory_edu_above)),
                   tab13 = sum(is.na(Compulsory_edu)),
...
## the first class, higher_edu
Journal_higher_edu <- Journal_edu2 %>% 
  dplyr::select(-c(5:9,12:15)) %>% 
  dplyr::filter(!is.na(Higher_edu) & !is.na(Below_higher_edu))

##  both > 1
Journal_higher_edu_integer <- Journal_higher_edu  %>%
  dplyr::filter(Higher_edu > 1 & Below_higher_edu > 1) %>%
  dplyr::summarise(Higher_edu = sum(Higher_edu),
                   Below_higher_edu = sum(Below_higher_edu))
  

## both < 1
Journal_higher_edu_decimal<- Journal_higher_edu  %>%
  dplyr::filter(Higher_edu <= 1 & Below_higher_edu <= 1) %>% 
  dplyr::mutate(Total_higher_edu = Higher_edu + Below_higher_edu) %>% 
  dplyr::mutate(Higher_edu_tab = floor(Sample_Size * Higher_edu),
                Below_higher_edu_tab = ifelse(Total_higher_edu == 1, Sample_Size - Higher_edu_tab, floor(Sample_Size * Below_higher_edu))) %>% 
  dplyr::mutate(tab = Higher_edu_tab + Below_higher_edu_tab) %>% 
  dplyr::summarise(Higher_edu = sum(Higher_edu_tab),
                   Below_higher_edu = sum(Below_higher_edu_tab))

## combine both class
Journal_higher_edu <-  bind_rows(Journal_higher_edu_decimal, Journal_higher_edu_integer) %>% 
  dplyr::summarise(Higher_edu = sum(Higher_edu),
                   Below_higher_edu = sum(Below_higher_edu)) %>% 
  tidyr::pivot_longer(cols = c(Higher_edu,Below_higher_edu),
...
## The second class, Compulsory_edu
Journal_compulsory_edu <- Journal_edu2 %>% 
  dplyr::select(4,12:13) %>% 
  dplyr::filter(!is.na(Compulsory_edu_above) & !is.na(Compulsory_edu))
# names(Journal_edu2)

##  both > 1
Journal_compulsory_edu_integer <- Journal_compulsory_edu  %>%
  dplyr::filter(Compulsory_edu_above > 1 & Compulsory_edu > 1) %>%
  dplyr::summarise(Compulsory_edu_above = sum(Compulsory_edu_above),
                   Compulsory_edu = sum(Compulsory_edu))
  

## both < 1
Journal_compulsory_edu_decimal<- Journal_compulsory_edu  %>%
  dplyr::filter(Compulsory_edu_above <= 1 & Compulsory_edu <= 1) %>% 
  dplyr::mutate(Total_Compulsory_edu = Compulsory_edu_above + Compulsory_edu) %>% 
  dplyr::mutate(Compulsory_edu_above_tab = floor(Sample_Size * Compulsory_edu_above),
                Compulsory_edu_tab = ifelse(Total_Compulsory_edu == 1, Sample_Size - Compulsory_edu_above_tab, floor(Sample_Size * Compulsory_edu))) %>% 
  dplyr::mutate(tab = Compulsory_edu_above_tab + Compulsory_edu_tab) %>% 
  dplyr::summarise(Compulsory_edu_above = sum(Compulsory_edu_above_tab),
                   Compulsory_edu = sum(Compulsory_edu_tab))

## combine both class
Journal_compulsory_edu <-  bind_rows(Journal_compulsory_edu_integer, Journal_compulsory_edu_decimal) %>% 
  dplyr::summarise(Compulsory_edu_above = sum(Compulsory_edu_above),
                   Compulsory_edu = sum(Compulsory_edu)) %>% 
  tidyr::pivot_longer(cols = c(Compulsory_edu_above,Compulsory_edu),
                      names_to = "Educational_Attainment",
                      values_to = "Num") %>% 
  dplyr::mutate(Proportion = round(Num/sum(Num),2)*100)

Journal_compulsory_edu


## The third class, edu_year
Journal_edu2_year <- Journal_edu2 %>% 
  dplyr::select(1,4,14:15) %>%
  dplyr::filter(!is.na(M_edu_year)) %>% 
  dplyr::mutate(M_edu_year = as.numeric(M_edu_year),
                Edu_year_cate = ifelse(M_edu_year <= 6, "Elementary School",
                                       ifelse(M_edu_year <= 9, "Middle School",
                                              ifelse(M_edu_year <= 12, "High School","University")))) %>% 
  dplyr::group_by(Edu_year_cate) %>% 
  dplyr::summarise(Sample_Size = sum(Sample_Size)) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round((Sample_Size/sum(Sample_Size)),2)*100) %>%
  # dplyr::add_row(Edu_year_cate = "Elementary School",
  #                Sample_Size = 0,
  #                Proportion = 0) %>% 
  dplyr::mutate(Edu_year_cate = factor(Edu_year_cate,
                                       levels = c("Middle School",
                                                   "High School","University")))

# sum(Journal_edu2_year$Sample_Size) # 5045
...
```{r}
## higher edu
H2_Fig_higher_edu <- ggplot(data = Journal_higher_edu,aes(Educational_Attainment,Proportion,fill = Educational_Attainment,Proportion))+
  geom_col()+
  theme_classic()+
  xlab("Educational attainment (21,407 subjects)")+
...
## compulsory edu
H2_Fig_compulsory_edu <- ggplot(data = Journal_compulsory_edu,aes(Educational_Attainment,Proportion,fill = Educational_Attainment,Proportion))+
  geom_col()+
  theme_classic()+
  xlab("Educational attainment (13,991 subjects)")+
  ylab("")+
  theme(legend.position = "none",    
        axis.title= element_text(size = 15,family = "serif"),
        axis.text = element_text(size = 10,family = "serif"),
...
H2_Fig_compulsory_edu


## edu year


H2_Fig_edu_year <- ggplot(data = Journal_edu2_year,aes(Edu_year_cate,Proportion,fill = Edu_year_cate))+
  geom_col()+
  xlab("Educational attainment (5,045 subjects)")+
  ylab("")+
  theme_classic()+
...
H2_Fig_edu <- H2_Fig_higher_edu + H2_Fig_compulsory_edu + H2_Fig_edu_year +
   plot_annotation(tag_levels = "A")  & 
