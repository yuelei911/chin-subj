...
H1_gender_general <- BTS_gender_CHN_general %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(273804,234047),
                 Proportion = c(54,46),Data_Source = "Journal")   ## Here, enter data manually.
...
# sum(H2_age_Psystages_bayes_general$`Chinese subjects pool`) ## 100
...
# sum(H2_age_inteval10_bayes_general$`Chinese subjects pool`) ## 100.0001 fill to percent 100?
...
H2_BF_edu_college_Census7 <- BayesMultiNomial(dataset = H2_edu_college_bayes,
                                          factor = "Educational_attainment",
                                          observed = "Chinese subjects pools",
                                          expected = "Census7")
...
# region_tab <- Journal_region_single_general %>% 
#   filter(Target_Population_N != 1 & Subjects_Recruitment_Area !=0) %>% 
#   distinct(Article_IDs) ## 129 papers
...
# sum(CFPS2018_region$Proportion) ## 100.0001 fil to percent 100?
```
...
Before starting the subsequent analysis, I need to remove those countries whose total sample size is less than 30.
```{r}
H3_country_filter <- BTS_Overview2 %>% 
  dplyr::filter(n > 30)

H3_country_filter <-  merge(H3_country_filter,Countrycodelist_iso2_iso3,by = "ISO2",all.x = TRUE)

H3_country_filter <- H3_country_filter %>% 
  dplyr::pull(ISO3)
```
...
BTS_gender_allcountry <- lapply(BTS_gender_list, function(df){
  ...
  df %>% 
    dplyr::select(ISO3,Data_Source,Gender) ## 这里应该增加Article_ID方便查找异常数据
...
BTS_gender_allcountry <- BTS_gender_allcountry %>% 
  dplyr::filter(ISO3 %in% H3_country_filter) %>% 
