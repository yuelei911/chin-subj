...
H3_male_more_China <- H3_gender %>% 
  dplyr::filter(Gender == "Male") %>%  ## the prop male of China is 46.46
  dplyr::filter(Proportion > 46.46)
```
...
H3_Lucca_gender_Journal <- H3_Lucca_gender %>% 
  ...
  dplyr::summarise(n = n()) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(n = ifelse(ISO3 == "CHN" & Gender == "Male", n+3825,n),
                n = ifelse(ISO3 == "CHN" & Gender == "Female", n+3590,n)) %>% 
...
H3_Lucca_gender_Journal2 <- H3_Lucca_gender_Journal %>% 
  # dplyr::left_join(WEIRD_Lucca_gender, by = c("ISO3" = "iso3c")) %>% 
