...
```{r message=FALSE, warning=FALSE, include=FALSE}
## The Mainland of China, Hong Kong of China, Macau of China, and Taiwan of China are assigned the unified value of China

b1 <- BTS_data[[8]]

## Fernandez-Velasco_et_al_2023 have TW CN AND HK

## Hoogeveen_et_al_2022 only mainland

BTS_data[[5]] <- BTS_data[[5]] %>% 
  dplyr::mutate(Country = ifelse(Country %in% c("Taiwan (Province of China) - TWN","Macao - MAC","Hong Kong - HKG"), 
                                 "China - CHN", Country))

BTS_data[[6]] <- BTS_data[[6]] %>% 
  dplyr::mutate(Country = ifelse(grepl("TWN |HKG ", Country),CHN,Country))
...
Journal_BTS_gender_CHN <- BTS_gender_CHN %>% 
  dplyr::mutate(Proportion = Proportion * 100) %>% 
...
Journal_BTS_gender_CHN_factor <- Journal_BTS_gender_CHN %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(Data_Source)

Journal_BTS_gender_CHN <- Journal_BTS_gender_CHN %>% 
  dplyr::mutate(Data_Source = factor(Data_Source,
                                     levels = Journal_BTS_gender_CHN_factor))
```


#### Visualize gender data for Chinese journals and BTS
```{r}
Journal_BTS_gender <- ggplot(data = Journal_BTS_gender_CHN,aes(Data_Source,Proportion,fill = Gender))+
  geom_col()+
...
ggsave("Journal_BTS_gender.pdf", Journal_BTS_gender, device = "pdf", width=18, height = 9)
...
#### Process age data from countries around the world in BTS
```{r}
## Arechar_et_al_2023
