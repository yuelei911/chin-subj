...
At this point, I have finished loading the package and data. Next, I analyzed the data and visualized it according to the logic of the written protocol.
...
Journal_SES_report_studynum <- Journal_SES_report %>% 
  ...
  dplyr::mutate(ses_reported_ceta = ifelse(grepl(";",SES_N),1,0))
...
## special
Journal_gender_special_papernum <-  Journal_gender2 %>% 
  dplyr::filter(Target_Population_N == 1) %>% 
  dplyr::distinct(Article_IDs)
