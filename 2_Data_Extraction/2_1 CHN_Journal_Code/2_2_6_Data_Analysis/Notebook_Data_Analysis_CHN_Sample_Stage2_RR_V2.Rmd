...
H3_female_more_China <- H3_gender %>% 
  dplyr::filter(Gender == "Female") %>%  ## the prop female of China is 53.5293
  dplyr::filter(Proportion > 53.5293)


H3_male_more_China <- H3_gender %>% 
  dplyr::filter(Gender == "Male") %>%  ## the prop male of China is 46.4707
  dplyr::filter(Proportion > 46.4707)
```
...
## The prop of 18~25 more than China (62.7359) 
H3_PsyStages2_more_China <- H3_PsyStages2 %>% 
  dplyr::filter(ageBins == "18~25") %>% 
  dplyr::filter(Proportion > 62.73596)
```
...
ggsave("H3_Fig_PsyStages.pdf",H3_Fig_PsyStages,device = "pdf", width=18, height = 18)
...
```{r}
H3_Fig_Lucca_gender <- ggplot(H3_Lucca_gender_Journal2,aes(x=Proportion, y=ISO3, fill = Gender))+
  geom_col(alpha = 0.75)+
