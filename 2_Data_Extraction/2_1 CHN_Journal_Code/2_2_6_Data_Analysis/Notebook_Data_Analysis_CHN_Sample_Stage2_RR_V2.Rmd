...
sum(Journal_age2$Sample_Size)  ## 359176
```
...
sum(BTS_age_general_CHN$Proportion) ## 100
...
sum(BTS_age_interval10_CHN_general$Proportion) ## 100.01
```
...
# sum(H1_interval10_bayes_general$Journal)  ## 100.01
# sum(H1_interval10_bayes_general$`BTS Chinese`) ## 100.01
...
Journal_edu2_tab <- Journal_edu_general %>% 
  ...
  dplyr::select(1,3,5) %>% 
  dplyr::rename(Class_higher_edu = 1, ## 20/185
                Class_compulsory_edu = 2, ## 25/185
                Class_edu_year = 3)  ## 48/185
```
...
# sum(Journal_edu2_year$Sample_Size) ## 0
...
BTS_college_CHN <- BTS_edu_CHN %>%
 ...
 # sum(BTS_college_CHN$Proportion) ## 100
...
# sum(BTS_highschool_CHN$Proportion) ## 100
...
H1_Fig_college <- ggplot(data = H1_edu_college,aes(Data_source,Proportion,fill = Educational_attainment))+
  ...
  # scale_fill_manual(values = c("Below_college" = "#F8766D",
  #                              "College_and_above" = "#00BFC4",
  #                              "Below_high_school"= "#7cae00",
  #                              "High_school_and_above" = "#619CFF"))+
