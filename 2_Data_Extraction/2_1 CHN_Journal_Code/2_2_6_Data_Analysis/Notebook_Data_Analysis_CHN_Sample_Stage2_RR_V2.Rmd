...
However, within the general population, I found there no different between BTS Chinese data and Chinese five journal data, with $Log(BF_{10})$ = `r H1_BF_gender_general$BF$LogBF10`.
...
Lucca_gender_special <- BTS[[19]] %>% 
  ...
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(n = n()) %>% ## Male 40, Female 53
  dplyr::ungroup() %>% 
...
# sum(H2_gender_preschool_special_prop$`Chinese subjects`)
