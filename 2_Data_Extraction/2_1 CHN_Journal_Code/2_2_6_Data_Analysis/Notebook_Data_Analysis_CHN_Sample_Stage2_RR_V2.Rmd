...
Journal <- df_stage2_Journal_Code2 %>% 
  ...
  dplyr::filter(Article_IDs_N != "20082359")
...
Journal_study_num <- Journal %>% 
  dplyr::distinct(Article_IDs,Study_Number) ## A total 1218 studies
...
sum(Journal_sample_size$Sample_Size,na.rm = TRUE)  ## 538,937
...
## Visualization
H2_region_four <- ggplot(data = H2_region_four2, aes(x = `Data Source`, y = Proportion, fill = Region_four)) +
...
ggsave("H2_region_four.pdf",H2_region_four, device = "pdf", width= 12, height = 9)
...
## Visualization
H2_region_seven <- ggplot(data = H2_region_seven2, aes(x = `Data Source`, y = Proportion, fill = Region_seven)) +
  geom_col(position = "stack")+
  labs(fill = "Region")+
  scale_x_discrete(labels =c("Journal_Prop" = "Journal",
                              "CFPS2018_Prop" = "CFPS2018",
...
ggsave("H2_region_seven.pdf",H2_region_seven, device = "pdf", width= 12, height = 9)
...
H2_region <- H2_Fig_Census7_region  + H2_Fig_region_general + H2_Fig_CFPS2018_region +
...
H2_region_four_bayes <- bind_rows(Journal_region_four,
  ...
  tidyr::pivot_wider(id_cols = Region_four,
                     ...
                     values_from = n) %>% 
  dplyr::mutate(Journal = Journal/sum(Journal),
                Census7 = Census7/sum(Census7),
                CFPS2018 = CFPS2018/sum(CFPS2018)) %>%  
...
sum(Journal_BTS_PsyStages_CHN$Proportion)  ## 100.01
...
## The prop of 18~25 more than China (60.72)
H3_PsyStages2_more_China <- H3_PsyStages2 %>% 
  dplyr::filter(ageBins == "18~25") %>% 
  dplyr::filter(Proportion > 60.72)
```
