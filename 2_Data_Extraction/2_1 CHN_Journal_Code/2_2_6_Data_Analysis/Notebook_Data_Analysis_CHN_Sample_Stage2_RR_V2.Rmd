...
# writexl::write_xlsx(Journal_special_target_population,"Journal_special_target_population.xlsx")
...
table(Journal_special_target_population2$Recode_target_population)


## num of different targetpopulation
Journal_special_adolecent <- Journal_special_target_population2 %>% 
  dplyr::filter(Recode_target_population == "儿童" | Recode_target_population == "青少年" | Recode_target_population == "儿童青少年") %>% 
...
# table(Journal_SES_report_studynum$ses_reported_ceta)
```


## the reporting situation of BTS
```{r}
# BTS_colnames_list <- lapply(names(BTS), function(name) {
#   data.frame(
#     source_dataframe = name,
#     column_name = colnames(BTS[[name]])
#   )
# })
# 
# # merge into one dataframe
# BTS_colnames <- do.call(rbind, BTS_colnames_list)
# 
# 
# ## exclude our add colnames
# BTS_colnames2 <- BTS_colnames %>% 
#   dplyr::filter(!column_name %in% c("ISO2","ISO3","Target_Population","Coding_Basis",
#                                  "Countrycode","Countryfullname","Coder")) %>% 
...
ggsave("H3_Fig_Lucca.pdf", H3_Fig_Lucca, device = "pdf", width=18, height = 9)
