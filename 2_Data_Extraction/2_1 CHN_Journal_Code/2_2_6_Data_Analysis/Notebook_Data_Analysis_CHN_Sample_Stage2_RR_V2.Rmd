...
```{r}
Fig_Journal_BTS_CHN_age <- ggplot(data= Journal_BTS_CHN_age, aes(x=ageBins, y=ifelse(Data_Source=="BTS Chineses", -Proportion, Proportion),fill = Data_Source)) +
...
H1_Fig_Journal_BTS_CHN <- Fig_Journal_BTS_CHN_gender + Fig_Journal_BTS_CHN_age +
  plot_annotation(tag_levels = 'A')

ggsave("H1_Fig_Journal_BTS_CHN.pdf", H1_Fig_Journal_BTS_CHN, device = "pdf", width=18, height = 9)
  ...
  if("Age" %in% names(BTS_data[[i]])){
    ...
    BTS_age <- BTS_data[[i]] %>% 
      ...
      dplyr::mutate(ageBins = cut(Age,
                                ...
                                levels = c("0~10","11~20","21~30", "31~40", "41~50", "51~60", ">=61")))
...
BTS_CHN_age_interval10_addrow <- BTS_CHN_age_interval10 %>% 
  dplyr::add_row(ageBins = "0~10", n = 0, Proportion = 0, Data_Source = "BTS CHN age")

Journal_BTS_CHN_interval10_bayes <- bind_rows(BTS_CHN_age_interval10_addrow,Journal_age_interval10) %>% 
  dplyr::select(-2) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion)
...
First, I create func to process age data simulation in psychological development stages.
```{r}
Sim_Sens_mult_PsyStages <- function(Mean1, diff_age, SD1, sample_N=Sample_Size){
  

  # 0~17, 18~25, 26~40, 41~60, 61~
  ageData1 <-  c(pnorm(17, mean = Mean1, sd = SD1) * sample_N,  
  ...
  sim_age_df <- setNames(data.frame(matrix(ncol = 2, nrow = 5)), c("ageBins", "sim1"))
  sim_age_df$ageBins <- c("<=17", "18~25", "26~40", "41~60", ">=61")
  sim_age_df$sim1[1:5] <- ageData1
...
## 
Sim_Journal_age_metrix <- lapply(1:nrow(Sim_Journal_age),function(i){
...
Sim_Journal_age_frame <- do.call(rbind,Sim_Journal_age_metrix)

Sim_Journal_age2 <- Sim_Journal_age_frame %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(sim1 = round(sum(sim1,na.rm = TRUE))) %>% 
  dplyr::ungroup()


sum(Sim_Journal_age$Sample_Size)-sum(Sim_Journal_age2$sim1,na.rm = TRUE)
```
...
Sim_Sens_mult_interval10 <- function(Mean2, diff_age2, SD2, sample_N2=Sample_Size){
  ...
  Mean3 <- Mean2 + diff_age2

  # 0~10, 11~20, 21~30, 31~40, 41~50, 51~60, 61~
  ageData2 <-  c(pnorm(10, mean = Mean2, sd = SD2) * sample_N2,
    ...
    # create a dataframe with ageData1 and ageData2:
  sim_age2_df <- setNames(data.frame(matrix(ncol = 2, nrow = 7)), c("ageBins", "sim2"))
  sim_age2_df$ageBins <- c("<=10", "11~20", "21~30", "31~40", "41~50", "51~60", ">=61")
  sim_age2_df$sim2[1:7] <- ageData2
...
Sim_Journal_age2_metrix <- lapply(1:nrow(Sim_Journal_age),function(i){
...
Sim_Journal_age2_frame <- do.call(rbind,Sim_Journal_age2_metrix)

Sim_Journal_age_interval10 <- Sim_Journal_age2_frame %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(sim2 = round(sum(sim2,na.rm = TRUE))) %>% 
  dplyr::ungroup()


sum(Sim_Journal_age$Sample_Size)-sum(Sim_Journal_age_interval10$sim2,na.rm = TRUE)
```
...
Fig_Journal_BTS_Census_age <- ggplot(data= df_census7_Age, 
  ...
  scale_color_manual(values=c("Chinese papers"="red", "BTS Chinese" = c("blue"))) +
...
Journal_region_distribution <- Census7_Area + CFPS2018_Area + Journal_Area + plot_layout(guides = "collect")+
...
BTS_All_Psystages_factor <- BTS_All_Psystages %>% 
...
Journal_BTS_ALL_PsyStages <- BTS_All_Psystages %>% 
  dplyr::left_join(WEIRD_code_gender, by = c("ISO3" = "iso3c")) %>% 
  dplyr::mutate(WEIRD = ifelse(ISO3 == "XKX","WEIRD",WEIRD)) %>%
  dplyr::mutate(ISO3 = factor(ISO3,BTS_All_Psystages_factor )) %>% 
...
```{r}
Fig_Journal_BTS_ALL_age <- ggplot(Journal_BTS_ALL_PsyStages, aes(x=Proportion, y=ISO3, fill=ageBins)) +
  geom_col(alpha = .75)+
  labs(y="Countries") + 
...
BTS_ALL_interval10_factor <- BTS_ALL_interval10 %>%
...
Journal_BTS_ALL_interval10 <- BTS_ALL_interval10 %>% 
  dplyr::left_join(WEIRD_code_gender, by = c("ISO3" = "iso3c")) %>% 
  dplyr::mutate(WEIRD = ifelse(ISO3 == "XKX","WEIRD",WEIRD)) %>%
  dplyr::mutate(ISO3 = factor(ISO3,BTS_ALL_interval10_factor)) %>% 
...
```{r}
Fig_Journal_BTS_ALL_interval10 <- ggplot(Journal_BTS_ALL_interval10, aes(x=Proportion, y=ISO3, fill=ageBins)) +
  geom_col(alpha = .75)+
  labs(y="Countries") + 
...
Fig_Journal_BTS_ALL_interval10 
```
...
```{r warning=FALSE}
Fig_BF_gender <- ggplot(BF_H3_gender_Journal_BTS, aes( x = `Log(BF10)_NonInform`, y = Countries)) +
  geom_point() +
  scale_color_grey() +
  xlim(-3, 25) +
