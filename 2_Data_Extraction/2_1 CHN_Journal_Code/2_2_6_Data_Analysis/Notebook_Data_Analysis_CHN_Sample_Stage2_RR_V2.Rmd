...
df_CHN_report <- do.call(rbind, report_list) %>% 
  ...
  dplyr::mutate(col_names = factor(col_names,
                                   labels = c("Religious","Ethnicity","Sampling Method",
                                              "Recruitment Area","Recruitment Method",
                                              "SES","Age","Gender"),
                                   levels = c("Religious","Ethnicity","Sampling_Method",
                                              "Recruitment_Area","Recruitment_Method",
                                              "SES","Age","Gender")))
...
df_fuzzy_report <- df_age_report_detail %>% 
  dplyr::filter(is.na(M_age)) 

## 20082227,20083017,20183340,20183097,20214027 Other age include Mage
##这里后面还需要重新处理一下部分Otherage包含Mage的情况

df_fuzzy_report_study_num <- df_fuzzy_report %>% 
  dplyr::distinct(Article_IDs,Study_Number) ## 134-5=129
```