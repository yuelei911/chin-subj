...
#   dplyr::count(ageBins) %>% 
#   dplyr::mutate(Proportion = round((n/sum(n))*100,2)) %>% 
#   dplyr::mutate(Data_Source = "Chinese papers",n=as.numeric(n))
# 
# 
...
```{r}
H1_Fig_age <- ggplot(data= Journal_BTS_CHN_age, aes(x=ageBins, y=ifelse(Data_Source=="BTS Chineses", -Proportion, Proportion),fill = Data_Source)) +
  geom_col(alpha=0.8, width = 1) +
  coord_flip() +
  labs(y="Proportion", x = "Age bins", color=NULL)+
  scale_fill_manual(values = c("BTS Chineses" = "#619CFF", "Chinese papers" = "#7CAE00")) +
  theme_classic()+
  theme(panel.border =element_rect(fill=NA,color="black"),
        legend.position ="bottom",
        legend.box.spacing = unit(2,"pt"),
...
## Set factor
H3_interval10_factor <- BTS_ALL_interval10 %>%
...
## Process WEIRD
H3_interval102 <- BTS_ALL_interval10 %>% 
  dplyr::left_join(WEIRD_code_gender, by = c("ISO3" = "iso3c")) %>% 
  dplyr::mutate(WEIRD = ifelse(ISO3 == "XKX","WEIRD",WEIRD)) %>%
...
```{r}
## PsyStages
BF_H3_all_PsyStages <- Journal_BTS_ALL_PsyStages %>% 
...
## interval10
BF_H3_all_interval10 <- Journal_BTS_ALL_interval10 %>% 
  dplyr::select(ISO3,ageBins,Proportion) %>% 
  dplyr::rename(Countries = 1)
...
BF_H3_interval10_Journal_BTS$CHN[7] <- 1

BF_H3_interval10_Journal_BTS <- BF_H3_interval10_Journal_BTS %>%
  Func_BFpairs(countries_order = BF_H3_gender_factor,
               prop_name = "ageBins")
