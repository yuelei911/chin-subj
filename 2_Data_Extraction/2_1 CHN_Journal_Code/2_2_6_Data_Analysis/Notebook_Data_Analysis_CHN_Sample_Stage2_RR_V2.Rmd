...
For different target population, I compared general population for them firstly, I found strong evidence for $H_1$ that the age distribution from the Chinese subjects data of BTS data is different form the Chinese jouranls data, the Bayesian multinomial test shows $Log(BF_{10})$ = `r H1_BF_PsyStages_general$BF$LogBF10`.
...
H2_age_bayes <- H2_age %>% 
  dplyr::select(-2) %>% 
  dplyr::mutate(Data_Source = "Chinese subjects pool") %>% 
  dplyr::bind_rows(Census7_Age) %>% 
  tidyr::pivot_wider(names_from = Data_Source,values_from = Proportion)

H2_BF_age <- BayesMultiNomial(dataset = H2_age_bayes,
                              factor = "ageBins",
                              observed = "Chinese subjects pool",
                              expected = "Census7")
...
Target_population_tab <-  Journal %>%
  ...
  dplyr::group_by(Article_IDs) %>% 
 dplyr::mutate(  
    unique_sample_types = n_distinct(Sample_Type_N),  # Check if Sample_Type_N is unique
    Study_Type = if_else(  
...
# Merge all unique Sample_Type_N values with toString, separated by "&"
  ) %>% 
  dplyr::slice(1) %>% 
  dplyr::ungroup() %>% 
...
Target_population_check <- Journal %>% 
  dplyr::select(Article_IDs,Target_Population_N,Coding_Basis_N) %>% 
  dplyr::filter(Target_Population_N == 1) %>% 
  dplyr::group_by(Article_IDs) %>% 
  dplyr::slice(1)
