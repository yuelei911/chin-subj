...
## total population
BTS_gender_CHN_total <- BTS_gender_CHN %>%
...
H1_gender_total <- BTS_gender_CHN_total %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(273804,234047),
                 Proportion = c(50,50),Data_Source = "Journal")   ## Here, enter data manually.

H1_gender_factor_total <- H1_gender_total %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(Data_Source)

H1_gender_total <- H1_gender_total %>% 
  dplyr::mutate(Data_Source = factor(Data_Source,
                                     levels = H1_gender_factor_total))


## General population
BTS_gender_CHN_general <- BTS_gender_CHN %>%
  dplyr::filter(Target_Population != 1) %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(n = n()) %>% 
  dplyr::mutate(Proportion = round(n/sum(n),2)*100) %>% 
  dplyr::ungroup() %>%
...
Total population
```{r}
H1_Fig_gender_total <- ggplot(data = H1_gender_total,aes(Data_Source,Proportion,fill = Gender))+
  geom_col()+
...
First, I calculate total population.
```{r}
H1_gender_bayes_total <- H1_gender_total %>% 
  dplyr::select(1,3:4) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion)

H1_BF_gender_total <- BayesMultiNomial(dataset = H1_gender_bayes_total,
                                             factor = "Gender",
                                             observed = "BTS Chinese",
                                             expected = "Journal")
```

I found moderate evidence for the $H_1$ that the gender proportion from Chinese five journal data is different from that of BTS data, with $Log(BF_{10})$ = `r H1_BF_gender_total$BF$LogBF10`.

and then, I compared the general population between the two.
```{r}
...
```{r}
rm(H1_gender_bayes_total,H1_gender_factor_total,H1_gender_factor_general,i,BTS_name)
```
...
## combine 2 dataframe
H1_age_total <- Sim_Journal_age_five3_prop %>%
  dplyr::rename(n = 2) %>%
  dplyr::bind_rows(BTS_age_total_CHN) ## 这里缺少>=95


## set ageBins as factor to plot
H1_age_total <- H1_age_total %>% 
  dplyr::mutate(ageBins = factor(ageBins,
                                 levels = c("0~4","5~9","10~14","15~19","20~24",
                                 "25~29","30~34","35~39","40~44","45~49",
                                 "50~54","55~59","60~64","65~69","70~74",
...
## calculate difference between of two data
H1_age_twosource_difference <- H1_age_total %>% 
  tidyr::pivot_wider(names_from = Data_Source,
                     values_from = c(Proportion,n)) %>% 
  dplyr::mutate(Differences = round(`Proportion_BTS Chinese`/ Proportion_Journal,2))
```


and then, I process the general population
```{r}
...
total 
```{r}
H1_Fig_age_total <- ggplot(data= H1_age_total, aes(x=ageBins, y=ifelse(Data_Source=="BTS Chinese", -Proportion, Proportion),fill = Data_Source)) +
...
# 
# H1_Fig_gender_age_total <- H1_Fig_gender_total + H1_Fig_age_total +
#   plot_annotation(tag_levels = 'A')
# 
# 
# ggsave("H1_Fig_gender_age_total.pdf", H1_Fig_gender_age_total, device = "pdf", width=18, height = 9)
# 
# 
# H1_Fig_gender_age_total
```
general
```{r}
...
### first, all data
BTS_age_PsyStages_CHN_total <- BTS_age_PsyStages %>% 
  dplyr::filter(ISO3 == "CHN") %>% 
  dplyr::select(1,5) %>% 
  dplyr::count(ageBins) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
...
### then, psychological development
...
### first, all data
BTS_age_interval10_CHN_total <- BTS_age_interval10 %>% 
  dplyr::filter(ISO3 == "CHN") %>% 
  dplyr::select(1,5) %>% 
  dplyr::count(ageBins) %>% 
  dplyr::mutate(Proportion = round((n/sum(n))*100,2),
...
```{r}
## all subjects
H1_PsyStages_bayes_total <- Sim_Journal_age_PsyStages3_prop %>% 
  dplyr::rename(n = 2) %>% 
  bind_rows(BTS_age_PsyStages_CHN_total) %>% 
  dplyr::select(-2) %>% 
  tidyr::pivot_wider(names_from = Data_Source, values_from = Proportion)

  
H1_BF_PsyStages_total <- BayesMultiNomial(dataset = H1_PsyStages_bayes_total,
                                          factor = "ageBins",
...
H1_interval10_bayes_total <- Sim_Journal_age_interval103_prop %>% 
  dplyr::rename(n = 2) %>% 
...
H1_BF_interval10_total <- BayesMultiNomial(dataset = H1_interval10_bayes_total,
                                          factor = "ageBins",
                                          observed = "BTS CHN age",
                                          expected = "Journal")
...
Note that there is not Bayes analysis here. 
...
H1_gender_total2 <- H1_gender_total %>% 
  dplyr::select(1:2) %>% 
  dplyr::group_by(Gender) %>% 
...
H2_gender_total <-  bind_rows(Census7_gender,CFPS2018_gender,H1_gender_total2)
...
total
```{r}
H2_Fig_gender_total <- ggplot(H2_gender_total,aes(Data_Source,Proportion,fill= Gender))+
  geom_col()+
  geom_hline(yintercept = 50, linetype = "dashed", color = "#666666")+
...
```{r}
## total
H2_gender_bayes_total <- H2_gender_total %>% 
  dplyr::select(2,3,1) %>% 
  tidyr::pivot_wider(names_from = Data_Source,values_from = Proportion)


H2_BF_gender_Census7_total <- BayesMultiNomial(dataset = H2_gender_bayes_total,
                                          factor = "Gender",
                                          observed = "Chinese subjects pool",
                                          expected = "Census7")


H2_BF_gender_CFPS2018_total <- BayesMultiNomial(dataset = H2_gender_bayes_total,
                                          factor = "Gender",
                                          observed = "Chinese subjects pool",
                                          expected = "CFPS2018")
...
total
```{r}
H2_Fig_age_total <- ggplot(data= Census7_age_five5, 
       aes(x=ageBins, y = Proportion, fill="Census7")) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = H2_age_total, aes(x=ageBins, 
...
H2_Fig_gender_age_total <- H2_Fig_gender_total + H2_Fig_age_total +
  plot_annotation(tag_levels = 'A')


ggsave("H2_Fig_gender_age_total.pdf", H2_Fig_gender_age_total, device = "pdf", width=18, height = 9)


H2_Fig_gender_age_total
```
general population
```{r}
...
H2_Fig_gender_age_general <- H2_Fig_gender_general + H2_Fig_age_general +
  plot_annotation(tag_levels = 'A')


ggsave("H2_Fig_gender_age_general.pdf", H2_Fig_gender_age_general, device = "pdf", width=18, height = 9)


H2_Fig_gender_age_general
```
...
## PsyStages Total
Sim_Journal_age_PsyStages3_prop <- Sim_Journal_age_PsyStages3_prop %>% 
  dplyr::rename(n =2)

BTS_age_PsyStages_CHN_total <- BTS_age_PsyStages_CHN_total %>% 
  dplyr::mutate(n = as.numeric(n))

H2_age_PsyStages_bayes_total<- bind_rows(BTS_age_PsyStages_CHN_total,Sim_Journal_age_PsyStages3_prop) %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(n = sum(n)) %>% 
  dplyr::mutate(Proportion = round(n/sum(n)*100,2),
...
## interval10
Sim_Journal_age_interval103_total_prop <-  Sim_Journal_age_interval103_prop %>% 
  dplyr::rename(n = 2)

H2_age_inteval10_bayes_total <- bind_rows(BTS_age_interval10_CHN_total,Sim_Journal_age_interval103_total_prop) %>% 
  dplyr::group_by(ageBins) %>% 
  dplyr::summarise(n = sum(n)) %>% 
  dplyr::mutate(Proportion = round(n/sum(n)*100,2),
                Data_Source = "Chinese subjects pool") %>% # sum(H2_age_general_bayes$Proportion) ## 100.01
...
## calculate BF
# PsyStages
H2_BF_age_PsyStages_total <- BayesMultiNomial(dataset = H2_age_PsyStages_bayes_total,
                                        factor = "ageBins",
                                        observed = "Chinese subjects pool",
                                        expected = "Census7")
...
## interval 10
H2_BF_age_interval10_total <- BayesMultiNomial(dataset = H2_age_interval10_bayes_total,
                                        factor = "ageBins",
                                        observed = "Chinese subjects pool",
                                        expected = "Census7")
...
For age of H2, firstly, in total, I found two age categories both have strong evidence for $H_1$ that the age distribution of Chinese subjects pool are different from Census7. The Bayesian multinomial test are PsyStages: $Log(BF_{10})$ = `r H2_BF_age_PsyStages_total$BF$LogBF10` and interval10: $Log(BF_{10})$ = `r H2_BF_age_interval10_total$BF$LogBF10` respectively.
...
```{r}
H2_edu_college <- bind_rows(Journal_college2,Census7_college,CFPS2018_edu_college)


H2_edu_high_school <- bind_rows(Journal_high_school2,Census7_high_school,CFPS2018_edu_high_school) %>% 
  dplyr::mutate(Data_source = factor(Data_source,
                                     levels = c("CFPS2018","Census7","Journal")))
```
...
H2_Fig_college <- ggplot(data = H2_edu_college,aes(Data_source,Proportion,fill = Educational_Attainment))+
  geom_col()+
  theme_classic()+ ## Journal num 4662+12092+9315
  theme(legend.position = "bottom",    
        axis.title= element_text(size = 15,family = "serif"),
        axis.text = element_text(size = 10,family = "serif"),
        strip.text = element_text(size = 15,family = "serif"),
...
## high school
H2_Fig_high_school <- ggplot(data = H2_edu_high_school,aes(Data_source,Proportion,fill = Educational_Attainment))+
  geom_col()+
  theme_classic()+  ## Journal Num 4662+9306+4685
  ylab("")+
  theme(legend.position = "bottom",    
        axis.title= element_text(size = 15,family = "serif"),
        axis.text = element_text(size = 10,family = "serif"),
...
sum(Journal_region_tab2$Proportion)
```
...
H2_Fig_region_total <- ggplot() +
  ...
  geom_sf(data = gjx) +
  labs(title = "Journal all subjects recruitment region distribution (290,585 subjects)") +
...
H2_region_bayes <- bind_rows(Journal_region_general_tab2,Journal_region_tab2,
...
H2_BF_Journal_Census7_region_total <- BayesMultiNomial(dataset = H2_region_bayes,
                                          factor = "Region",
                                          observed = "Journal",
                                          expected = "Census7")


H2_BF_Journal_CFPS2018_region_total <- BayesMultiNomial(dataset = H2_region_bayes,
                                          factor = "Region",
                                          observed = "Journal",
                                          expected = "CFPS2018")
