...
Journal <- df_stage2_Journal_Code2 %>% 
  ...
  dplyr::filter(!grepl("2",Subjects_Recruitment_Area)) ## valid 1569 rows

Journal_articles_num <- Journal %>% 
  dplyr::distinct(Article_IDs) ## A total 963 articles
...
Journal_study_num <- Journal %>% 
  dplyr::distinct(Article_IDs,Study_Number) ## A total 1219 studies
...
# sum(Journal_sample_size$Sample_Size,na.rm = TRUE)  ## 538,949
...
table(Target_population_special_gender$Sample_Type_N)

# ## combine tow target population
...
Journal_gender_Infants_toddlers_TP <- Journal_special_target_population2 %>% 
  ...
  dplyr::filter(!grepl("幼儿父亲|幼儿教师|母亲|8|10|11|12",Recode_target_population))
  
## merge the above data.frame with "Journal" data.frame
Journal_gender_Infants_toddlers_TP_IDs <- Journal %>% 
  dplyr::filter(Article_IDs_N %in% Journal_Infants_toddlers_TP$Article_IDs) %>% 
  ...
  dplyr::left_join(Journal_Infants_toddlers_TP,
...
Journal_gender_Infants_toddlers <- Journal_gender_Infants_toddlers_TP_IDs %>% 
  dplyr::summarise(Male_Num = sum(as.numeric(Male_Num)),
                   ...
                   Male_Proportion = round(Male_Num/(Male_Num+Female_Num)*100,2),
                   Female_Proportion = round(Female_Num/(Male_Num+Female_Num)*100,2)) ## Male 51.58, Female 48.42
```
...
H1_gender_general <- BTS_gender_CHN_general %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(55366,45480),
                 Proportion = c(54.90,45.10),Data_Source = "Journal")   ## Here, enter data manually.
...
Lucca_gender_special <- BTS[[21]] %>% 
  ...
  dplyr::mutate(Porportion = round(n/sum(n)*100,2), ## Male 50.64, Female 49.36
                Data_Source = "Lucca") %>% 
  dplyr::select(1,3,4)
