...
Journal_gender_college_sepcial2 <- Journal_gender_college_sepcial %>% 
  ...
  tidyr::pivot_longer(cols = c("Male","Female"),
                      names_to = "Gender",
                      values_to = "n") %>% 
  dplyr::mutate(Proportion = round(n/sum(n)*100,2)) ## Male 44.95 / Female 55.05
...
# sum(H2_age_general$Proportion) ## 100
```
...
sum(Census7_age_PsyStages$Proportion) #100.01
...
sum(H2_age_Psystages_bayes_general$`Chinese subjects`) ## 100
...
sum(H2_age_inteval10_bayes_general$`Chinese subjects`) ## 100.0001 fill to percent 100?
...
H2_edu_highschool <- bind_rows(Chinese_subjects_pools_edu_highschool,Census7_high_school,CFPS2018_edu_high_school)  %>%
  dplyr::mutate(Data_source = factor(Data_source,
                                     levels = c("CFPS2018","Census7","Chinese subjects")))
```
...
sum(Journal_region_general_tab2$n) #43009
```
