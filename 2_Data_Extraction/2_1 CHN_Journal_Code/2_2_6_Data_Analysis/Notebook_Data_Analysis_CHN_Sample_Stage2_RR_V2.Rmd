...
report <- ggplot(df_CHN_report,aes(col_names,proportion,fill=Var1))+
  ...
  scale_fill_manual(values = c("#adc6ad","#B2B2B2"))

report

ggsave("report.pdf", report, device = "pdf", width=16, height = 9)
...
### Abstract1 subjects information reported
```{r message=FALSE, warning=FALSE}
...
## CHN Journa and BTS' CN gender
```{r}

## laod data
df_Arechar_et_al_2023 <- read.csv("Arechar_et_al_2023.csv")
df_Azevedo_et_al_2022_mainland <- read.csv("pro_Azevedo_et_al_2022_mainland.csv")
df_Azevedo_et_al_2022_taiwan <- read.csv("pro_Azevedo_et_al_2022_taiwan.csv")
df_Bosson_et_al_2021 <- read.csv("pro_Bosson_et_al_2021.csv")
df_Buchanan_et_al_2023 <- read.csv("pro_Buchanan_et_al_2023.csv")
df_Dorison_et_al_2020 <- read.csv("pro_Dorison_et_al_2020.csv")


## Precess data for gender


## CHN Journal
df_CHN_Journal_gender <- df_CHN_Journal_Gender_Total %>% 
...
##Arechar_et_al_2023
df_Arechar_et_al_2023_gender <- df_Arechar_et_al_2023 %>% 
  dplyr::filter(Country == "cn") %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Arechar_et_al_2023")


## Azevedo_et_al_2022_mainland
df_Azevedo_et_al_2022_mainland_gender <- df_Azevedo_et_al_2022_mainland %>%
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Azevedo_et_al_2022_mainland")


## Azevedo_et_al_2022_taiwan
df_Azevedo_et_al_2022_taiwan_gender <-  df_Azevedo_et_al_2022_taiwan %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Azevedo_et_al_2022_taiwan")


## Bosson_et_al_2021
df_Bosson_et_al_2021_gender <- df_Bosson_et_al_2021 %>% 
  dplyr::filter(Country == "China") %>% 
  dplyr::filter(Gender == "Male" | Gender == "Female") %>%  ## gender has multiple categories.
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Bosson_et_al_2021")


## Buchanan_et_al_2023
df_Buchanan_et_al_2023_gender <- df_Buchanan_et_al_2023 %>% 
  dplyr::filter(grepl("China | Hong Kong | Taiwan | Macao", Country)) %>% 
  dplyr::filter(Gender == "Female" | Gender == "Male") %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Buchanan_et_al_2023")

## Dorison_et_al_2020
df_Dorison_et_al_2020_gender <- df_Dorison_et_al_2020 %>% 
  dplyr::filter(grepl("CHN | HKG | TWN",Country)) %>% 
  dplyr::filter(Gender == "Female" | Gender == "Male") %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(count = n()) %>%
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(count/sum(count),2),
                Data_Source = "Dorison_et_al_2020")
  


df_CHNJournal_BTS_gender <- bind_rows(df_Azevedo_et_al_2022_mainland_gender,df_Azevedo_et_al_2022_taiwan_gender,
                                      df_Arechar_et_al_2023_gender,df_CHN_Journal_gender,
                                      df_Bosson_et_al_2021_gender,df_Buchanan_et_al_2023_gender,
                                      df_Dorison_et_al_2020_gender) %>% 
  dplyr::mutate(Gender = str_to_title(Gender))


CHNJournal_BTS_gender <- ggplot(data = df_CHNJournal_BTS_gender,aes(Data_Source,Proportion,fill = Gender))+
  geom_col()+
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "#666666")+
  labs(y="Proportion")+
  theme_classic()+
  theme(panel.border =element_rect(fill=NA,color="black"),
        legend.position = "bottom",
...
ggsave("CHNJournal_BTS_gender.pdf", CHNJournal_BTS_gender, device = "pdf", width=18, height = 9)

CHNJournal_BTS_gender
```


### CHN Journal and BTS gender in other region/country
```{r}
Func_Process_gender <- function(data){
  ## Function is designed to preliminarily process the data and sort it based on gender ratios, in order to facilitate subsequent visualization.
  
  ## data is input data.frame
 
   data_valid_country <- data %>% 
    dplyr::group_by(iso3c) %>% 
    dplyr::summarise(count = n()) %>%
    dplyr::filter(iso3c > 30 & !is.na(iso3c)) %>% 
    dplyr::pull(iso3c)
   
   
   data <- data %>% 
     dplyr::filter(iso3c %in% data_valid_country) %>% 
     dplyr::group_by(iso3c,Gender) %>%
     dplyr::summarise(count = n()) %>% 
     dplyr::ungroup() %>% 
     dplyr::group_by(iso3c) %>% 
     dplyr::mutate(Proportion = round(count/sum(count),2)) %>% 
  ...
  data_country <- data %>% 
    ...
    dplyr::arrange(Gender,Proportion) %>% 
    dplyr::pull(iso3c)
  
  data <- data %>% 
    dplyr::mutate(iso3c = factor(iso3c,levels = data_country))
  
}

## example 1
df_Arechar_et_al_2023_Oth_gender <- df_Arechar_et_al_2023 %>%  
  dplyr::mutate(Country = toupper(Country)) %>% 
  dplyr::left_join(countrycodelist_iso2_iso3, by = c("Country" = "iso2c")) %>% 
  Func_Process_gender()

## example 2


df_Arechar_et_al_2023_Oth_gender %>% 
  ggplot(aes(Proportion, iso3c,fill = Gender)) +
           geom_col()+
  ylab("Country")+
  theme_classic()+
...
## 国家编码和WEIRD编码确定后完善数据处理和可视化  

```

## CHN Journal and BTS age
```{r}
Function_Process_age <- function(data){
  ...
  ### Create a data frame to complete the missing ageBins 
 
  Age_bins <- df_census7_Age %>% 
  dplyr::select(1) %>% 
  dplyr::mutate(n = NA, Proportion = NA , Data_Source = NA) 
  
  
  data_age <- data_age %>% 
  dplyr::full_join(Age_bins, by = "ageBins") %>% 
  dplyr::rename(n = 2, Proportion = 3) %>% 
  dplyr::select(1:3) %>% 
  dplyr::mutate(n = ifelse(is.na(n),0,n),
                Proportion = ifelse(is.na(Proportion),0,Proportion))
  
  
## Turning ageBins into factor variables for easy plotting
  agebins_code <- Age_bins %>% 
    dplyr::arrange(ageBins) %>% 
    dplyr::pull(ageBins)
...
## Arechar_et_al_2023
df_Arechar_et_al_2023_age <- df_Arechar_et_al_2023 %>% 
  dplyr::filter(Country == "cn") %>% 
  Function_Process_age()


Arechar_et_al_2023_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = df_Arechar_et_al_2023_age, aes(x=ageBins, 
                y = Proportion, group = 1, color ="Arechar_et_al_2023"),size =1)+
  scale_fill_manual(values=c("Census 7"="#00BFC4")) +  
  scale_color_manual(values=c("Arechar_et_al_2023"="red")) +
...
#ggsave("Arechar_et_al_2023_age.pdf", Arechar_et_al_2023_age, device = "pdf", width=18, height = 9)

Arechar_et_al_2023_age



## Azevedo_et_al_2022_mainland
df_Azevedo_et_al_2022_mainland_age <- df_Azevedo_et_al_2022_mainland %>% 
  Function_Process_age()


Azevedo_et_al_2022_mainland_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = df_Azevedo_et_al_2022_mainland_age, aes(x=ageBins, 
                y = Proportion, group = 1, color ="Azevedo_et_al_2022_mainland"),size =1)+
  scale_fill_manual(values=c("Census 7"="#00BFC4")) +  
  scale_color_manual(values=c("Azevedo_et_al_2022_mainland"="red")) +
  scale_y_continuous(limits = c(0,20))+
  coord_flip() +
  labs(y="Proportion", x = "Age bins")+
  theme_classic()+
...
#ggsave("Azevedo_et_al_2022_mainland_age.pdf", Azevedo_et_al_2022_mainland_age, device = "pdf", width=18, height = 9)

Azevedo_et_al_2022_mainland_age 



## Azevedo_et_al_2022_taiwan_age
df_Azevedo_et_al_2022_taiwan_age <- df_Azevedo_et_al_2022_taiwan %>% 
  Function_Process_age()


Azevedo_et_al_2022_taiwan_age <- ggplot(data= df_census7_Age, 
       aes(x=ageBins, y = Proportion, fill="Census 7")) +
  geom_col(alpha=0.5, width = 1) +
  geom_line(data = df_Azevedo_et_al_2022_taiwan_age, aes(x=ageBins, 
                y = Proportion, group = 1, color ="Azevedo_et_al_2022_taiwan"),size =1)+
  scale_fill_manual(values=c("Census 7"="#00BFC4")) +  
  scale_color_manual(values=c("Azevedo_et_al_2022_taiwan"="red")) +
  scale_y_continuous(limits = c(0,20))+
  coord_flip() +
  labs(y="Proportion", x = "Age bins")+
  theme_classic()+
...
#ggsave("Azevedo_et_al_2022_taiwan_age.pdf", Azevedo_et_al_2022_taiwan_age, device = "pdf", width=18, height = 9)

Azevedo_et_al_2022_taiwan_age 

age_plot <-  Arechar_et_al_2023_age + Azevedo_et_al_2022_mainland_age + 
  Azevedo_et_al_2022_taiwan_age +
  plot_layout(ncol = 2, nrow = 2,heights = c(4,4))+
  patchwork::plot_annotation(tag_levels = 'A')

ggsave("age_plot.pdf", age_plot, device = "pdf", width=18, height = 9)
...
## The distrbution of subjects recruitment region about CHN Journal and BTS 
```{r}
...
df_Arechar_et_al_2023_subj_region <- df_Arechar_et_al_2023 %>% 
  dplyr::group_by(Country) %>% 
  dplyr::summarise(n=n()) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(Proportion = round(n/sum(n),2)) %>%  ## sum only 0.98, need process
...
Arechar_et_al_2023_map <- ggplot2::ggplot(df_Arechar_et_al_2023_subj_region)+
  geom_map(aes(map_id = toupper(Country), fill = Proportion_bins), map = world_map2,show.legend = TRUE) +
...
ggtitle("Geographical distribution of sample from Arechar_et_al_2023")+
...
sample_map <- Arechar_et_al_2023_map + world_population_map + 
