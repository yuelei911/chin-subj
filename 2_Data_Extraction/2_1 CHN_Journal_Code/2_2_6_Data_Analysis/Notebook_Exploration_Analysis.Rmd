...
## psy2021
ShanghaiRanking_psy2021_provincetab <- ShanghaiRanking_psy2021 %>% 
  dplyr::rename(Region = 4) %>% 
  dplyr::group_by(Region) %>% 
  dplyr::summarise(psy2021 = n()) %>% 
  dplyr::ungroup()

## applied psy
ShanghaiRanking_appliedpsy2021_provincetab <- ShanghaiRanking_appliedpsy2021 %>% 
  dplyr::rename(Region = 4) %>% 
  dplyr::group_by(Region) %>% 
  dplyr::summarise(appliedpsy2021 = n()) %>% 
  dplyr::ungroup()
...
Region_psy_cordata <- left_join(China_province,ShanghaiRanking_psy2021_provincetab,by = "Region") %>% 
  left_join(Journal_region_ShanghaiRanking,by = "Region")


## applied psy merge data
Region_appliedpsy_cordata <- left_join(China_province,ShanghaiRanking_appliedpsy2021_provincetab,by = "Region") %>%
  left_join(Journal_region_ShanghaiRanking,by = "Region")
...
```{r}
cor.test(Region_psy_cordata$psy2021, Region_psy_cordata$Journal, method = "pearson", use = "pairwise.complete.obs")
...
```{r}
cor.test(Region_appliedpsy_cordata$appliedpsy2021, Region_appliedpsy_cordata$Journal, method = "pearson", use = "pairwise.complete.obs")
