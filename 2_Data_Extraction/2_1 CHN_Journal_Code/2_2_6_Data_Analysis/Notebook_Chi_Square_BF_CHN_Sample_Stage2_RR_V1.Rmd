...
The chi-square value-based bayes factor follow by Liu, Ghai, Azevedo, et al.(2025), https://sciety.org/articles/activity/10.31234/osf.io/kufbt_v6
...
H1_PsyStages_bayes_general <- H1_PsyStages_bayes_general %>% 
  dplyr::mutate(Scaling_factor = 400/sum(`BTS Chinese`),
                `BTS Chinese New` = floor(Scaling_factor * `BTS Chinese`),
                Dif = 400 - sum( `BTS Chinese New`),
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_PsyStages_general$parameter,thresh = 4^4)
...
H1_interval10_bayes_general <- H1_interval10_bayes_general %>% 
  dplyr::mutate(Scaling_factor = 200/sum(`BTS Chinese`),
                `BTS Chinese New` = floor(Scaling_factor * `BTS Chinese`),
                Dif = 200 - sum( `BTS Chinese New`),
                `BTS Chinese New` = ifelse(row_number() == 1,
                                             `BTS Chinese New` + Dif,
                                             `BTS Chinese New`))
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_interval10_general$parameter,thresh = 4^2)
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_college$parameter,thresh = 4^40)
...
## calculate BF
lambda <- UMPBT(df = H1_Chi_highschool$parameter,thresh = 4^40)
...
H2_college_bayes_general <- H2_college_bayes_general %>% 
  dplyr::mutate(Scaling_factor = 600/sum(`Chinese subjects`),
                `Chinese subjects New` = floor(Scaling_factor * `Chinese subjects`),
                Dif = 600 - sum( `Chinese subjects New`),
                `Chinese subjects New` = ifelse(row_number() == 1,
                                             `Chinese subjects New` + Dif,
                                             `Chinese subjects New`))
```
...
## calculate BF
lambda <- UMPBT(df = H2_Chi_college_bayes_general$parameter,thresh = 4^12)
...
H2_highschool_bayes_general <- H2_highschool_bayes_general %>% 
  dplyr::mutate(Scaling_factor = 350/sum(`Chinese subjects`),
                `Chinese subjects New` = floor(Scaling_factor * `Chinese subjects`),
                Dif = 350 - sum( `Chinese subjects New`),
                `Chinese subjects New` = ifelse(row_number() == 1,
                                             `Chinese subjects New` + Dif,
                                             `Chinese subjects New`))
```
...
## calculate BF
lambda <- UMPBT(df = H2_Chi_highschool_bayes_general$parameter,thresh = 4^21)
...
## calculate BF
lambda <- UMPBT(df = H2_Chi_highschool_bayes_general$parameter,thresh = 4^7)
...
H3_gender_CHN <- H3_gender %>% 
  dplyr::filter(ISO3 == "CHN") %>% 
  dplyr::select(ISO3,Gender,n,Proportion) %>% 
  dplyr::mutate(Scaling_factor = 200/sum(n),
...
for (ISO3 in countries) {
  ...
  ## calculate BF
  lambda <- UMPBT(df = H3_Chi_gender$parameter,thresh = 4^1)
...
H3_PsyStages_CHN <- H3_PsyStages2 %>% 
  dplyr::filter(ISO3 == "CHN") %>% 
  dplyr::select(ISO3,ageBins,n,Proportion) %>% 
  dplyr::mutate(Scaling_factor = 100/sum(n),
                n_New = floor(Scaling_factor * n),
                Dif = 100 - sum(n_New),
                n_New = ifelse(row_number() == 1,
                                             n + Dif,
...
for (ISO3 in countries) {
  ...
  ## calculate BF
  lambda <- UMPBT(df = H3_Chi_PsyStages$parameter,thresh = 4^1)
