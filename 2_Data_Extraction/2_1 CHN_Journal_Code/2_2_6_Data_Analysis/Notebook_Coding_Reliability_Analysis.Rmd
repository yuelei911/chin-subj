...
Coder2 <- Chin_Subj_CE[seq(2,nrow(Chin_Subj_CE), by = 3), ]
```


## process Coder1 and  Coder NA
```{r}
Coder1 <- Coder1 %>%
...
Coder1$Abstract2 <- as.factor(Coder1$Abstract2)
Coder2$Abstract2 <- as.factor(Coder2$Abstract2)

## Target population
Coder1$Target_Population_N <- as.factor(Coder1$Target_Population_N)
Coder2$Target_Population_N <- as.factor(Coder2$Target_Population_N)
...
## gender reported
Coder1$Gender <- as.factor(Coder1$Gender)
Coder2$Gender <- as.factor(Coder2$Gender)

## age 
Coder1$Age <- as.factor(Coder1$Age)
Coder2$Age <- as.factor(Coder2$Age)

## educational attainment
table(Coder1$Educational_Attainment_N)
Coder1$Educational_Attainment_N <- as.factor(Coder1$Educational_Attainment_N)
Coder2$Educational_Attainment_N <- as.factor(Coder2$Educational_Attainment_N)

## remark1 & 2
Coder1 <- Coder1 %>% 
  dplyr::mutate(Remark1_N = case_when(Remark1_N == "animal" ~ "1", 
                                      Remark1_N == "mining" ~ "2",
                                      Remark1_N == "qualitative" ~ "3", 
                                      Remark1_N == "secondary" ~ "4",
                                      Remark1_N == "NA" ~ "NA",
                                      TRUE ~ "5"),  ## other inappropriate uses
                Remark1_N = as.factor(Remark1_N))
...
Coder1 <- Coder1 %>%
  dplyr::mutate(Remark2_N = case_when(
      # Code as 3 if both "区" and "重复" are present
      grepl("区", Remark2_N) & grepl("重复", Remark2_N) ~ "3",
      # Code as 2 if only "区" is present
      grepl("区", Remark2_N) ~ "2",
      # Code as 1 if only "重复" is present
      grepl("重复", Remark2_N) ~ "1",
      # Code as NA for all other cases
      TRUE ~ "NA"),
      Remark2_N = as.factor(Remark2_N))
...
Coder1$Subjects_Recruitment_Method_N <- as.factor(Coder1$Subjects_Recruitment_Method_N)

Coder2$Subjects_Recruitment_Method_N <- Coder2$Subjects_Recruitment_Method_N %>% 
  recode("1;2" = "1")
Coder2$Subjects_Recruitment_Method_N <- as.factor(Coder2$Subjects_Recruitment_Method_N)

## sampling method
Coder1$Sampling_Method <- as.factor(Coder1$Sampling_Method)
Coder2$Sampling_Method <- as.factor(Coder2$Sampling_Method)
```
...
irrs_continuous <- data.frame(
  IRR = sapply(vars, function(i) 
    kripp.alpha(rbind(Coder1[[i]], Coder2[[i]]), "ratio")$value  ## value represent kripp.alpha
  ),
  check.names = FALSE
)
...
irrs_categorical <- data.frame(
  IRR = sapply(vars, function(i) 
    kripp.alpha(rbind(Coder1[[i]], Coder2[[i]]), "nominal")$value  ## value represent kripp.alpha
  ),
  check.names = FALSE
)
...
```{r}
agreement_percent <- data.frame(
  `percentage of agreement` = sapply(vars, function(i) {
  
    combined <- cbind(Coder1[[i]], Coder2[[i]])
...
```{r}
merge_Age <- cbind(Coder1$Age,Coder2$Age)
