...
BTS_region_CHN_total <- BTS_region_CHN %>% 
...
# sum(BTS_region_CHN_total$Proportion)
```
...
H1_Journal_Region_data <- Journal_region_general_tab2 %>%
...
H1_BTS_Region_data <- BTS_region_CHN_total %>%
  dplyr::left_join(Region_map_color,by = join_by(Region)) %>% 
  dplyr::mutate(QUHUADAIMA = ifelse(Region == "重庆", "500000", QUHUADAIMA)) %>% 
  tidyr::pivot_wider(names_from = Data_Source,values_from = c(Proportion, Count_Bins,n,Color)) %>% 
  dplyr::mutate(across(3:ncol(.),~ifelse(is.na(.),0,.)))
...
China_Journal <- dplyr::left_join(China,H1_Journal_Region_data,by= "QUHUADAIMA")
China_BTS <- dplyr::left_join(China,H1_BTS_Region_data,by = "QUHUADAIMA")
...
H2_BTS_region <- BTS_region_CHN_total %>% 
  dplyr::select(Region,n)
...
## combine data
H2_region_four <- bind_rows(H2_Journal_region_four,H2_BTS_region_four) %>% 
  tidyr::pivot_longer(cols = -Region_four,
                      names_to = "Data source",
                      values_to = "n") %>% 
  dplyr::group_by(Region_four) %>% 
...
H2_region_seven <- bind_rows(H2_Journal_region_seven,H2_BTS_region_seven) %>% 
  tidyr::pivot_longer(cols = -Region_seven,
                      names_to = "Data source",
                      values_to = "n") %>% 
  dplyr::group_by(Region_seven) %>% 
