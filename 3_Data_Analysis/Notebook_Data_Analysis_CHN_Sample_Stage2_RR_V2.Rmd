...
**Note:** For all analyses in this document, Log(BFa0) denotes the natural logarithm (ln) of BFa0, as specified in the custom function.
...
$H_0$: $\theta = c$, where $c$ is defined by Chinese five journal data; $H_a$: $\theta$ is free to vary.
...
However, within the general population, I found there is no difference between BTS Chinese data and Chinese five journal data, with $Log(BF_{a0})$ = `r H1_BF_gender_general$BF$LogBFa0`.
...
H1_BF_gender_special <-  BayesMultiNomial(dataset = H1_gender_bayes_special,
                                          ...
                                          expected = "Journal")
```
In the special population of infants and toddlers, I did not find difference between Lucca et al. (2024) and Chinese five journal data, with $Log(BF_{a0})$ = `r H1_BF_gender_special$BF$LogBFa0`.
...
H1_BF_PsyStages_general <- BayesMultiNomial(dataset = H1_PsyStages_bayes_general,
                                          ...
                                          expected = "Journal")
```
For age (Psychological stages) with general population, there is difference between BTS and Chinese papers, $Log(BF_{a0})$ = `r H1_BF_PsyStages_general$BF$LogBFa0`.
...
I also found extreme evidence support $H_a$ for another age category,the Bayesian multinomial test shows $Log(BF_{a0})$ = `r H1_BF_interval10_general$BF$LogBFa0`.
...
**suppl edu analysis**:In above analysis, educational data from Chinese journals were included only when the participants were adults (i.e., sample type code 4 or 5). Following reviewer suggestions, I also include other types of participants whose target population is the general population.
...
I found moderate evidence for the $H_0$ that the college proportion from Chinese five journal data is comparable to that in the BTS data, with $Log(BF_{a0})$ = `r H1_BF_edu_college$BF$LogBFa0`.
...
I also found moderate evidence for the $H_0$ that the high schools proportion from Chinese five journal data is comparable to that in the BTS data, with $Log(BF_{a0})$ = `r H1_BF_edu_highschool$BF$LogBFa0`.
...
Then, I firstly process Journal data.
...
Next, I process the BTS data. We only have the general population data of BTS
```{r}
...
Trans into four regions and seven regions
```{r}
## four regions
...
combine map data and research data
```{r}
...
For four regions, I found extreme evidence for the $H_a$ that the BTS is different from that of Journal data, with $Log(BF_{a0})$ = `r H1_BF_Journal_BTS_region_four$BF$LogBFa0`, as well as equally the extreme evidence for the seven regions $Log(BF_{a0})$ = `r H1_BF_Journal_BTS_region_seven$BF$LogBFa0`


Visualize the major regions
```{r}
...
The calculation results show that there is no difference between the data in Chinese participants and Census7 and CFPS2018, and the calculation results are: Census7 VS Chinese participants: $Log(BF_{a0})$ = `r H2_BF_gender_Census7_general$BF$LogBFa0` , CFPS2018 VS Chinese participants: $Log(BF_{a0})$ = `r H2_BF_gender_CFPS2018_general$BF$LogBFa0`
...
H2_BF_gender_preschool_Census <- BayesMultiNomial(dataset = H2_gender_preschool_bayes_special,
                                                ...
                                                expected = "Census7")
```
Gender:Lucca vs Journal, $Log(BF_{a0})$ = `r H2_BF_gender_preschool_Census$BF$LogBFa0`. The result showed that moderate evidence in favor of $H_0$
...
In terms of gender, for the special target population of college students, we have found that there is no difference between Journal and the overall data of China (the Outline for Women's Development in China (2021-2030)),  $Log(BF_{a0})$ = `r H2_BF_gender_college_special$BF$LogBFa0`.
...
For age within general population, I found the Bayesian multinomial test shows extreme evidence in foavor of $H_a$. PsyStages and interval10 are $Log(BF_{a0})$ = `r H2_BF_age_Psystages_general$BF$LogBFa0` and $Log(BF_{a0})$ = `r H2_BF_age_interval10_general$BF$LogBFa0` respectively.
...
H2_BF_edu_highschool_CFPS2018 <- BayesMultiNomial(dataset = H2_edu_highschool_bayes,
                                          ...
                                          expected = "CFPS2018")
```
I found all results show extreme evidence support $H_a$. 

College: Census7($Log(BF_{a0})$ = `r H2_BF_edu_college_Census7$BF$LogBFa0`), CFPS2018($Log(BF_{a0})$ = `r H2_BF_edu_college_CFPS2018$BF$LogBFa0`),
High school: Census7($Log(BF_{a0})$ = `r H2_BF_edu_highschool_Census7$BF$LogBFa0`), CFPS2018($Log(BF_{a0})$ = `r H2_BF_edu_highschool_CFPS2018$BF$LogBFa0`)


### Region
Merge Journal and BTS Chinese data
...
I processed of provincial distribution of population for census7.
...
Then, I processed of provincial distribution of population for CFPS2018.
```{r}
...
H2_BF_Chinese_participants_CFPS2018_region_four <- BayesMultiNomial(dataset = H2_region_four_bayes,
                                          ...
                                          expected = "CFPS2018")
```
four regions: Journal vs Census7,$Log(BF_{a0})$ =`r H2_BF_Chinese_participants_Census7_region_four$BF$LogBFa0`
four regions: Journal vs CFPS2018,$Log(BF_{a0})$ =`r H2_BF_Chinese_participants_CFPS2018_region_four$BF$LogBFa0`
...
H2_BF_Chinese_participants_CFPS2018_region_seven <- BayesMultiNomial(dataset = H2_region_seven_bayes,
                                          ...
                                          expected = "CFPS2018")
```
seven regions: Journal vs Census7,$Log(BF_{a0})$ =`r H2_BF_Chinese_participants_Census7_region_seven$BF$LogBFa0`
seven regions: Journal vs CFPS2018,$Log(BF_{a0})$ =`r H2_BF_Chinese_participants_CFPS2018_region_seven$BF$LogBFa0`
