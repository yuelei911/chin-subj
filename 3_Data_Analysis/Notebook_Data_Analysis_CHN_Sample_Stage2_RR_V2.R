...
load(here("3_Data_Analysis","3_1_Intermediate_Data","Journal.Rdata")) 

# "Stage2_Journal_Code.Rdata" includes Journal coding data, replaced articles coding data, and preprocessed data for "other age".


load(here("3_Data_Analysis","3_1_Intermediate_Data","BTS.Rdata"))  

# "BTS.Rdata" includes data of each international collaborative project, BTS preprocessed education data, and BTS target population coded data.


load(here("3_Data_Analysis","3_1_Intermediate_Data","Analyze_supporting_data.Rdata")) 

#"Analyze_supporting_data" contains educational and regional data from census6 and census7, ShanghaiRanking data, country code data, and color data for regional plotting.
...
BayesMultiNomial <- function(dataset, factor, observed, expected, default_prior = TRUE, prior = NA){
  ...
  if (any(rowSums(cbind(thetas, counts)) == 0)) {
    LogBFa0 <- (lbeta.xa-lbeta.a)
  } else {
     LogBFa0 <- (lbeta.xa-lbeta.a) + (0 - sum(counts * log(thetas))) 
  }

  BF <- data.frame(LogBFa0 = LogBFa0,
                   BFa0    = exp(LogBFa0),
                   BF0a    = 1/exp(LogBFa0))
...
# writexl::write_xlsx(Journal_special_population,"Journal_special_population.xlsx")
...
Journal_college_TP <- Journal_special_target_population2 %>% 
  ...
  dplyr::filter(Recode_target_population == "大学生" | Recode_target_population == "研究生")
  # %>%
...
sum(BTS_Overview2$Proportion) ## 100
sum(BTS_Overview2$n) ## 1367084
29006/1367084


writexl::write_xlsx(BTS_Overview2,here("2_Data_Extraction","2_2_BTS","BTS_participants.xlsx"))
...
Journal_education_occupation_report <- Journal %>% 
...
table(Journal_education_occupation_report$Educational_Attainment_N) ## FALSE 166 TRUE 140
table(Journal_education_occupation_report$Occupation_N)  ## FALSE 201 TRUE 105


Journal_education_occupation_table <- data.frame(Var1 = c("Unreported","Reported",
                                                            "Unreported","Reported"),
                                                   Freq = c(166,140,201,105),
                                                   col_names= c("Educational_Attainment",
                                                                "Educational_Attainment",
                                                                "Occupation","Occupation"),
                                                   proportion = c(166/(166+140),140/(166+140),201/(201+105),105/(201+105)))


Journal_report <- rbind(Journal_report,Journal_education_occupation_table) %>%
  dplyr::mutate(col_names = factor(col_names, 
                                   labels = c("Sex","Age","Recruitment method",
                                              "Recruitment area","Educational attainment",
                                              "Occupation","Sampling method",
                                              "Socioeconomic status","Ethnicity","Religious belief"),
                                   levels = c("Sex","Age","Recruitment_Method",
                                              "Recruitment_Area","Educational_Attainment",
                                              "Occupation","Sampling_Method",
                                              "SES","Ethnicity","Religion")),
...
## articles
Journal_age_fuzzy_report_article_num <- Journal_age_fuzzy_report %>% 
  dplyr::distinct(Article_IDs) ## 120-6=114

## study
Journal_age_fuzzy_report_study_num <- Journal_age_fuzzy_report %>% 
  dplyr::distinct(Article_IDs,Study_Number)  ## 140-6=134
...
Journal_SES_report_studynum <- Journal_SES_report %>% 
  dplyr::distinct(Article_IDs,Study_Number,SES_N) %>% 
  dplyr::filter(Article_IDs != "20081025" & Article_IDs != "20183264") %>%  ## 2
  dplyr::mutate(ses_reported_ceta = ifelse(grepl(";",SES_N),1,0))  ## 339+2=341


## SES information is 5
Journal_SES_report_5 <- Journal_SES_report %>% 
  dplyr::distinct(Article_IDs,Study_Number,SES_N) %>% 
  dplyr::filter(Article_IDs != "20081025" & Article_IDs != "20183264") %>% 
...
Journal_gender <- Journal_key_variable %>% 
  dplyr::select(-c(10:17)) %>% ## not select age, remake and subjects recruitment area
  dplyr::filter(Gender == 1)

## Given that gender-related data includes both proportion-based records and headcount figures, these two types of data were processed separately.
...
## general
Journal_gender_general_papernum <- Journal_gender2 %>% 
  dplyr::filter(Target_Population_N != 1) %>% 
  dplyr::distinct(Article_IDs)
...
Journal_gender_Infants_toddlers_TP <- Journal_special_target_population2 %>% 
  ...
  dplyr::filter(grepl("幼儿|岁儿童|学步儿|学前|学龄|低龄|婴儿", Recode_target_population)) %>% 
  dplyr::filter(!grepl("幼儿父亲|幼儿教师|母亲|学前儿童父母|8|10|11|12",Recode_target_population))
  

## table paper num for targeted infants/todders
Journal_gender_Infants_toddlers_TP_tab <- Journal_gender_Infants_toddlers_TP %>% 
  dplyr::distinct(Article_IDs)
...
BTS_gender_CHN <- lapply(BTS_gender_list, function(df){
  ...
  df %>% 
    ...
    dplyr::filter(Data_Source != "BTS_Lucca_et_al_2024") %>%  ## no select special population
    dplyr::select(ISO3,Data_Source,Gender,Target_Population) 

})

BTS_gender_CHN_general <- do.call(rbind,BTS_gender_CHN) %>% 
...
## Combine Chinese journal data and set factor
H1_gender_general <- BTS_gender_CHN_general %>% 
  dplyr::add_row(Gender = c("Female","Male"), n = c(57570,48059),
                 Proportion = c(54.50208,45.49792),Data_Source = "Journal")   ## Here, enter data manually, data from data.frame "Journal_gender_general".
...
Lucca_gender_special <- BTS[[19]] %>% 
  ...
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(n = n()) %>% ## Female 40, Male 53
  dplyr::ungroup() %>% 
  dplyr::filter(!is.na(Gender)) %>% 
  dplyr::mutate(Porportion = n/sum(n)*100, 
                Data_Source = "Lucca")
...
## Lucca'data from data.frame "Lucca_gender_special"
## Journal'data from data.frame "Journal_gender_Infants_toddlers"
H1_gender_bayes_special <- data.frame(Gender = c("Female","Male"),
                                      Lucca = c(43.01075,56.98925), 
                                      Journal = c(48.4253,51.5747))
...
Journal_strings_age_newdata <- Journal_strings_age %>% 
...
Sim_Sens_mult_PsyStages <- function(Mean1, diff_age, SD1, sample_N=Sample_Size){
  ...
  ageData_PsyStages <-  c(pnorm(17, mean = Mean1, sd = SD1) * sample_N,
                          (pnorm(25, mean = Mean1, sd = SD1) - pnorm(17, mean = Mean1, sd = SD1)) *sample_N,
                          (pnorm(40, mean = Mean1, sd = SD1) - pnorm(25, mean = Mean1, sd = SD1)) *sample_N,
...
Sim_Sens_mult_interval10 <- function(Mean2, diff_age2, SD2, sample_N2=Sample_Size){
  ...
  ageData_interval10 <-  c(pnorm(10, mean = Mean2, sd = SD2) * sample_N2,
                           (pnorm(20, mean = Mean2, sd = SD2) - pnorm(10, mean = Mean2, sd = SD2)) *sample_N2,
                           (pnorm(30, mean = Mean2, sd = SD2) - pnorm(20, mean = Mean2, sd = SD2)) *sample_N2,
                           (pnorm(40, mean = Mean2, sd = SD2) - pnorm(30, mean = Mean2, sd = SD2)) *sample_N2,
                           (pnorm(50, mean = Mean2, sd = SD2) - pnorm(40, mean = Mean2, sd = SD2)) *sample_N2,
                           (pnorm(60, mean = Mean2, sd = SD2) - pnorm(50, mean = Mean2, sd = SD2)) *sample_N2,
                           (1 - pnorm(60, mean = Mean2, sd = SD2)) * sample_N2  
)
...
## paper num
Journal_college_papernum <- Journal_college %>% 
  dplyr::distinct(Article_IDs)
...
## papers num
Journal_highschool_papernum <- Journal_edu_general %>% 
  dplyr::filter(!is.na(High_school_and_above) & !is.na(Below_high_school)) %>% 
  dplyr::distinct(Article_IDs)
...
H1_edu_college <- H1_edu_college %>% 
  dplyr::mutate(Educational_attainment = ifelse(Educational_attainment == "Below_college",
                                                "Less-than-college","College-or-above"),
                Educational_attainment = factor(Educational_attainment,
                                                levels = c("College-or-above","Less-than-college")))


H1_edu_highschool <- H1_edu_highschool %>% 
  dplyr::mutate(Educational_attainment = ifelse(Educational_attainment == "Below_high_school","Less-than-high-school","High-school-or-above"),
                Educational_attainment = factor(Educational_attainment,
...
## Article 20185082' Sample_Size became NA during data format conversion, but it does not affect the results as it is unrelated to the target population(special population).
...
#    dplyr::filter(Subjects_Recruitment_Area != "0" & grepl(";",Subjects_Recruitment_Area)) 
 ...
 Journal_region_multiple <-  Journal_region_multiple %>% 
...
Journal_region_four <- merge(Journal_region_general_tab2,China_region_four,
  ...
  dplyr::mutate(Journal = (Journal_n/sum(Journal_n))*100)

# sum(Journal_region_four$Journal)
...
BTS_region_CHN <- lapply(BTS_region, function(df){
  ...
  df <- df %>% 
    dplyr::filter(Country == "China" | Country == "CHN" | Country == "CN") %>% 
    
  return(df)
  
})

BTS_region_CHN <- do.call(rbind,BTS_region_CHN)


BTS_region_CHN <- BTS_region_CHN %>% 
  dplyr::filter(!Province %in% c("China","Other")) %>% 
  ...
  dplyr::mutate(Count_Bins = cut(Proportion,
                                 breaks = c(-Inf,1,5,10,Inf),
                labels = c("<1","1~5","5~10",">=10")),
                Data_Source = "BTS region general",
                Color = ifelse(Count_Bins == "<1",1,
                               ifelse(Count_Bins == "1~5",2,
                                      ifelse(Count_Bins == "5~10",3,4))),
                Color = factor(Color, levels = c("1", "2", "3", "4")))
...
BTS_region_CHN_four <- BTS_region_CHN %>% 
  ...
  dplyr::mutate(`BTS Chinese` = (n/sum(n))*100)


# sum(BTS_region_CHN_four$`BTS Chinese`)

## seven regions
BTS_region_CHN_seven <- BTS_region_CHN %>% 
  merge(China_region_seven,by= "Region") %>% 
  dplyr::group_by(Region_seven) %>% 
  dplyr::summarise(n=sum(n)) %>% 
  dplyr::mutate(`BTS Chinese` = (n/sum(n))*100)
...
BTS_region_CHN_four_bayes <- BTS_region_CHN_four %>% 
  dplyr::select(Region_four,`BTS Chinese`) %>% 
  dplyr::add_row(Region_four = "Northeast China",
                 `BTS Chinese` = 0)


H1_region_four_bayes <- left_join(Journal_region_four_bayes,
                                  BTS_region_CHN_four_bayes,
                                  by = "Region_four")

## BF
H1_BF_Journal_BTS_region_four <- BayesMultiNomial(dataset = H1_region_four_bayes,
                                          factor = "Region_four",
                                          observed = "BTS Chinese",
                                          expected = "Journal")
...
BTS_region_CHN_seven_bayes <- BTS_region_CHN_seven %>% 
  dplyr::select(Region_seven,`BTS Chinese`) %>% 
  dplyr::add_row(Region_seven = "Northeast China",
                 `BTS Chinese` = 0)


H1_region_seven_bayes <- left_join(Journal_region_seven_bayes,
                                  BTS_region_CHN_seven_bayes,
                                  by = "Region_seven")

## BF
H1_BF_Journal_BTS_region_seven <- BayesMultiNomial(dataset = H1_region_seven_bayes,
                                          factor = "Region_seven",
                                          observed = "BTS Chinese",
                                          expected = "Journal")
...
H1_Fig_region_four <- ggplot(data = H1_region_four_plot, aes(x = `Data source`, y = Proportion, fill = Region_four)) +
...
H1_region_seven_plot <- H1_region_seven_bayes %>% 
  tidyr::pivot_longer(cols = -Region_seven,
                      names_to = "Data source",
                      values_to = "Proportion") %>% 
  dplyr::mutate(Region_seven = factor(Region_seven,
                                      levels = c("South China","North China",
                                                 "East China","Southwest China",
                                                 "Central China","Northwest China",
                                                 "Northeast China")))


H1_Fig_region_seven <- ggplot(data = H1_region_seven_plot, aes(x = `Data source`, y = Proportion, fill = Region_seven)) +
  geom_col(position = "stack")+
  labs(fill = "Region")+
  theme_classic()+
  theme(legend.box.spacing = unit(2,"pt"),
...
## calculate Prop
H2_gender_preschool_special_prop <- H2_gender_preschool_special %>% 
  dplyr::group_by(Gender) %>% 
  dplyr::summarise(n=sum(n)) %>% 
  dplyr::mutate(Proportion = n/sum(n)*100,
                 Data_source = "Chinese participants")

# sum(H2_gender_preschool_special_prop$Proportion) ## 100
# sum(H2_gender_preschool_special$n) ## 7523
...
# sum(Census7_gender_preschool_special$Proportion) ## 100


## combine tow data 
H2_gender_preschool_bayes_special <- bind_rows(H2_gender_preschool_special_prop,
...
## bayes 
H2_BF_gender_preschool_Census <- BayesMultiNomial(dataset = H2_gender_preschool_bayes_special,
                                                factor = "Gender",
                                                observed = "Chinese participants",
                                                expected = "Census7")
...
Journal_gender_college_sepcial <-  Journal %>% 
  ...
  dplyr::mutate(Male_Num = as.numeric(Male_Num),
                ...
                Male_Num = ifelse(Male_Num < 1 & Female_Num < 1,
                                  floor(Male_Num * Sample_Size),Male_Num),
...
## The total data we compared is from *the Outline for Women's Development in China (2021-2030)* released by the National Bureau of Statistics of China in 2022. website:https://www.stats.gov.cn/sj/zxfb/202312/t20231229_1946062.html

H2_gender_college_bayes_special <- data.frame(Gender = c("Male","Female"),
                                              Journal = c(45.4552,54.5448),
...
Census6_age <- df_census6 %>% 
  ...
  dplyr::select(Age,n) %>% 
  dplyr::slice(7:126) %>%  ## select agebins rows
  dplyr::filter(!grepl("岁",Age)) %>% 
  dplyr::mutate(Age = as.numeric(Age),
                n = as.numeric(n),
                n = ifelse(Age == "99",n+35934,n))  ## include 100 years old and above


## PsyStages
Census6_age_PsyStages <- Census6_age %>% 
  dplyr::mutate(ageBins = cut(Age,
...
sum(Census6_age_PsyStages$Proportion)


## interval10
Census6_age_interval10 <- Census6_age %>% 
  dplyr::mutate(ageBins = cut(Age,
...
Census7_age <- df_census7 %>%
  dplyr::rename(ageBins = 1, n = 2) %>% 
  dplyr::slice(9:145) %>% 
  dplyr::filter(ageBins != "" & !grepl("岁",ageBins)) %>% 
  dplyr::select(1,2) %>% 
  dplyr::mutate_at(vars(1:2),as.numeric) %>% 
  dplyr::mutate(n = ifelse(ageBins == "99", n+118866,n))  ## include 100 years old and above
...
sum(H2_age_Psystages_bayes_general$`Chinese participants`) 
...
sum(H2_age_inteval10_bayes_general$`Chinese participants`) 
...
Census7_college <- Census7_edu_adults %>% 
  dplyr::summarise(`Less-than-college` = sum(across(3:7)),
                   `College-or-above` = sum(across(8:11))) %>% 
...
Census7_high_school <- Census7_edu_adults %>% 
  dplyr::summarise(`Less-than-high-school` = sum(across(3:6)),
                   `High-school-or-above` = sum(across(7:11))) %>% 
  tidyr::pivot_longer(cols = c(1,2),
                      names_to = "Educational_attainment",
                      values_to = "Num") %>% 
  dplyr::mutate(Proportion = Num/sum(Num)*100,
...
CFPS2018_edu_college <- data.frame(Educational_attainment = c("Less-than-college","College-or-above"),
...
CFPS2018_edu_high_school <- data.frame(Educational_attainment = c("Less-than-high-school","High-school-or-above"),
                                   Num = c(2846+756+3598+5273+3058,1648+1335+112+7)) %>% 
  dplyr::mutate(Proportion = Num/sum(Num)*100,
                Data_source = "CFPS2018")
...
H2_edu_college <- bind_rows(Chinese_subjects_pools_edu_college,
                            Census7_college,CFPS2018_edu_college)
...
H2_edu_highschool <- bind_rows(Chinese_subjects_pools_edu_highschool,
...
H2_Fig_college <- ggplot(data = H2_edu_college,aes(Data_source,Proportion,fill = Educational_attainment))+
  ...
  scale_fill_manual(values = c("Less-than-college" = "#00BFC4", "College-or-above" = "#F8766D")) +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 16, family = "serif"),
        legend.title = element_blank(),
        axis.title= element_text(size = 16,family = "serif"),
...
H2_Fig_highschool <- ggplot(data = H2_edu_highschool,aes(Data_source,Proportion,fill = Educational_attainment))+
  ...
  scale_fill_manual(values = c("Less-than-high-school" = "#00BFC4", "High-school-or-above" = "#F8766D")) +
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 16, family = "serif"), 
        legend.title = element_blank(),
        axis.title= element_text(size = 16,family = "serif"),
...
H2_BTS_region_four <- BTS_region_CHN_four %>% 
  dplyr::select(Region_four,n) %>% 
  dplyr::rename(`BTS Chinese` = 2)

## combine data
H2_BTS_Journal_region_four <- bind_rows(H2_Journal_region_four,H2_BTS_region_four) %>% 
  tidyr::pivot_longer(cols = -Region_four,
                      names_to = "Data source",
                      values_to = "n") %>% 
  dplyr::group_by(Region_four) %>% 
...
## region seven
H2_Journal_region_seven <- Journal_region_seven %>% 
  dplyr::select(Region_seven,Journal_n) %>% 
  dplyr::rename(Journal = 2)

H2_BTS_region_seven <- BTS_region_CHN_seven %>% 
  dplyr::select(Region_seven,n) %>% 
  dplyr::rename(`BTS Chinese` = 2)

H2_BTS_Journal_region_seven <- bind_rows(H2_Journal_region_seven,H2_BTS_region_seven) %>% 
  tidyr::pivot_longer(cols = -Region_seven,
                      names_to = "Data source",
                      values_to = "n") %>% 
  dplyr::group_by(Region_seven) %>% 
...
H2_region_four <- full_join(H2_BTS_Journal_region_four,Census7_region_four,
  ...
  dplyr::mutate(Chinese_participants_Prop = Chinese_participants_n / sum(Chinese_participants_n)*100,
...
## BF
H2_BF_Chinese_participants_Census7_region_four <- BayesMultiNomial(dataset = H2_region_four_bayes,
                                          factor = "Region_four",
                                          observed = "Chinese participants",
                                          expected = "Census7")

H2_BF_Chinese_participants_CFPS2018_region_four <- BayesMultiNomial(dataset = H2_region_four_bayes,
                                          factor = "Region_four",
                                          observed = "Chinese participants",
                                          expected = "CFPS2018")
...
## BF
H2_BF_Chinese_participants_Census7_region_seven <- BayesMultiNomial(dataset = H2_region_seven_bayes,
                                          factor = "Region_seven",
                                          observed = "Chinese participants",
                                          expected = "Census7")

H2_BF_Chinese_participants_CFPS2018_region_seven <- BayesMultiNomial(dataset = H2_region_seven_bayes,
                                          factor = "Region_seven",
                                          observed = "Chinese participants",
                                          expected = "CFPS2018")
...
H3_BTS_gender <- BTS_gender_allcountry %>% 
  dplyr::filter(ISO3 %in% BTS_gender_allcountry_filter) %>% ## filter > 30
  dplyr::filter(Data_Source != "BTS_Lucca_et_al_2024") %>% ## remove special target population
...
## Chinese journal data please see data.frame "H1_gender_general"
H3_BTS_Journal_gender  <- H3_BTS_gender  %>% 
  dplyr::mutate(n = ifelse(ISO3 == "CHN" & Gender == "Male", 13833 + 48059,n),
                n = ifelse(ISO3 == "CHN" & Gender == "Female", 13723 + 57570,n))

H3_BTS_Journal_gender_prop <- H3_BTS_Journal_gender %>% 
  dplyr::group_by(ISO3) %>% 
  dplyr::mutate(Proportion = n/sum(n)*100) %>%
  dplyr::ungroup()


H3_BTS_Journal_gender_sum_prop <- H3_BTS_Journal_gender_prop %>% 
  dplyr::group_by(ISO3) %>% 
...
H3_BTS_Journal_gender_factor <- H3_BTS_Journal_gender_prop %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(ISO3)
...
WEIRD_gender <- WEIRD %>% 
  ...
  dplyr::filter(iso3c %in% H3_BTS_Journal_gender_factor)


H3_gender <- H3_BTS_Journal_gender_prop %>% 
...
Journal_BTS_PsyStages_CHN <- H2_age_Psystages_bayes_general %>% 
  ...
  dplyr::rename(Proportion = `Chinese participants`) %>% 
  dplyr::select(-Census7)
...
H3_PsyStages2 <- H3_PsyStages %>% 
  ...
  dplyr::rename(`Age bins` = 2)


## The prop of 18~25 more than 60%
...
Func_BFpairs <- function(df, countries_order, prop_name){
  ...
  colnames(BF_mult_tmp) <- c("Observed", "Expected", "Log(BFa0)_NonInform", "BFa0_NonInform", "BF0a_NonInform")
  ...
  for (ii in seq(length(countries_order))){
    ...
    if (country != "CHN"){
      ...
      BF_mult_tmp[ii,] <- c(as.character(country), "CHN", 
                            BF_tmp$BF$LogBFa0, BF_tmp$BF$BFa0, BF_tmp$BF$BF0a)
    } 
  }
  
  countries_order_age <- BF_mult_tmp %>% 
    tidyr::drop_na() %>%
    dplyr::mutate(`Log(BFa0)_NonInform` = as.numeric(`Log(BFa0)_NonInform`)) %>%
    dplyr::arrange(`Log(BFa0)_NonInform`) %>%
    dplyr::pull(Observed)
  ...
  BF_mult_p_tmp <- BF_mult_tmp %>%
    ...
    dplyr::mutate(Countries = factor(Observed, levels = countries_order_age),
                  `Log(BFa0)_NonInform` = as.numeric(`Log(BFa0)_NonInform`)) 
...
H3_BF_gender <- H3_BF_gender  %>% 
  ...
  Func_BFpairs(countries_order = H3_BF_gender_factor,
               prop_name = "Sex") %>% 
  dplyr::rename("Countries/Regions" = "Countries")

## moderate evidence support H0
H3_BF_gender_supportH0 <- H3_BF_gender %>% 
  dplyr::filter(`Log(BFa0)_NonInform` < log(1/6))

## moderate evidence support Ha
H3_BF_gender_supportHa <- H3_BF_gender %>% 
  dplyr::filter(`Log(BFa0)_NonInform` > log(6))

## female and male
H3_gender_different <- H3_gender %>% 
  dplyr::filter(ISO3 %in% H3_BF_gender_supportHa$Observed) %>% 
  dplyr::filter(Sex == "Female")
...
H3_Fig_BF_gender <- ggplot(H3_BF_gender, aes( x = `Log(BFa0)_NonInform`, y = `Countries/Regions`)) +
  ...
  geom_text(aes(x=log(10), label="\nStrong evidence for Ha", y = 23), colour="blue", alpha = 0.7, 
            angle=90, text=element_text(size=10)) +
  geom_text(aes(x=log(6), label="Moderate evidence for Ha\n", y = 23), colour="deepskyblue", alpha = 0.7, 
            angle=90, text=element_text(size=10)) +
...
table(H3_BF_PsyStages$`Age bins`)
...
H3_BF_PsyStages <- H3_BF_PsyStages %>%  
  dplyr::mutate(Countries = factor(Countries, 
                                             levels = H3_BF_PsyStages_factor)) %>%  
  tidyr::pivot_wider(names_from = "Countries",values_from = "Proportion") %>%
...
H3_BF_interval10_factor <- H3_BF_interval10 %>% 
  dplyr::filter(`Age bins` == "11~20") %>%
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(Countries)


table(H3_BF_interval10$`Age bins`)


H3_BF_interval10 <- H3_BF_interval10 %>% 
  dplyr::mutate(Countries = factor(Countries, 
                                   levels = H3_BF_interval10_factor)) %>% 
  tidyr::pivot_wider(names_from = "Countries",values_from = "Proportion") %>%
...
H3_Fig_BF_PsyStages <- ggplot(H3_BF_PsyStages, aes( x = `Log(BFa0)_NonInform`, 
  ...
  geom_text(aes(x=log(10), label="\nStrong evidence for Ha", y = 23), colour="blue", alpha = 0.7, 
            angle=90, text=element_text(size=10)) +
  geom_text(aes(x=log(6)+1, label="Moderate evidence for Ha\n", y = 23), colour="deepskyblue", alpha = 0.7, 
            angle=90, text=element_text(size=10)) + ## add 1.2 avoid label overlap
  geom_text(aes(x=log(1/6)-1, label="\nModerate evidence for H0", y = 23), colour="#D55E00", alpha = 0.7, 
            angle=90, text=element_text(size=10)) +
  geom_text(aes(x=log(1/10), label="Strong evidence for H0\n", y = 23), colour="orange", alpha = 0.7, 
            angle=90, text=element_text(size=10))  +
  xlab("Log(BFa0)")+
...
## calculate CHN in Lucca et al.(2024)
H3_Lucca_gender_CHN <- H3_Lucca_gender %>% 
  dplyr::filter(ISO3 == "CHN") %>% 
  dplyr::summarise(n = n())


## ## filter n > 30
H3_Lucca_gender_filter30 <- H3_Lucca_gender %>% 
  dplyr::group_by(ISO3) %>% 
  dplyr::summarise(n = n()) %>% 
  dplyr::ungroup() %>% 
  dplyr::filter(n > 30) %>% 
...
## add journal gender data
## Journal within target population is infants/toddlers please see data.frame "Journal_gender_Infants_toddlers"

H3_Lucca_gender_Journal <- H3_Lucca_gender %>% 
  dplyr::filter(ISO3 %in% H3_Lucca_gender_filter30) %>% 
   dplyr::group_by(ISO3,Gender) %>% 
  dplyr::summarise(n = n()) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(n = ifelse(ISO3 == "CHN" & Gender == "Male", n+3832,n),
                n = ifelse(ISO3 == "CHN" & Gender == "Female", n+3598,n)) %>% 
  dplyr::filter(!is.na(ISO3) & !is.na(Gender)) %>% 
  dplyr::group_by(ISO3) %>% 
  dplyr::mutate(Proportion = n/sum(n)*100)


## arrange data by prop
H3_Lucca_gender_Journal_factor <- H3_Lucca_gender_Journal %>% 
  dplyr::filter(Gender == "Female") %>% 
  dplyr::arrange(Proportion) %>% 
  dplyr::pull(ISO3)
...
# ## combine WEIRD data
# WEIRD_Lucca_gender <- WEIRD %>% 
#   dplyr::filter(!is.na(iso3c)) %>% 
#   dplyr::filter(iso3c %in% H3_Lucca_gender_Journal_factor)

H3_Lucca_gender_Journal2 <- H3_Lucca_gender_Journal %>% 
  dplyr::mutate(ISO3 = factor(ISO3, levels = H3_Lucca_gender_Journal_factor)) %>% 
...
H3_BF_Lucca_gender2 <- H3_BF_Lucca_gender %>% 
  dplyr::mutate(Countries = factor(Countries, levels = H3_Lucca_gender_Journal_factor)) %>% 
  tidyr::pivot_wider(names_from = "Countries", values_from = "Proportion") %>% 
  Func_BFpairs(countries_order = H3_Lucca_gender_Journal_factor,
               prop_name = "Gender")


H3_Fig_BF_Lucca_gender <- ggplot(H3_BF_Lucca_gender2, 
                                 aes( x = `Log(BFa0)_NonInform`, y = Countries)) +
  geom_point() +
  scale_color_grey() +
  xlim(-3, 3) +
  scale_y_discrete(limits=rev) +
  geom_vline(xintercept = c(log(1/10), log(1/6), 0, log(6), log(10)), 
             colour = c("orange", "#D55E00", "black", "deepskyblue","blue"), 
             linetype=c("longdash", "longdash", "solid", "longdash","longdash")) + 
  geom_text(aes(x=log(10), label="\nStrong evidence for Ha", y = 5), colour="blue", alpha = 0.7, 
            angle=90, text=element_text(size=10)) +
...
H3_Fig_BF_Lucca_gender


## 
H3_Fig_Lucca <- H3_Fig_Lucca_gender + H3_Fig_BF_Lucca_gender +
  plot_annotation(tag_levels = 'A')  & 
  theme(plot.tag = element_text(size = 18, face = "bold"))
